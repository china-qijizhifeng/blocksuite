...
import { almostEqual, isPointOnLineSegment, lineEllipseIntersects, lineIntersects, linePolygonIntersects, linePolylineIntersects, pointAlmostEqual, polygonGetPointTangent, rotatePoints, toDegree, toRadian, } from '../../surface-block/utils/math-utils.js';
describe('Line', () => {
    it('should intersect', () => {
    ...
    it('lineEllipseIntersects', () => {
        ...
        assertExists(rst);
        expect(rst.every((point, index) => pointAlmostEqual(point, expected[index]))).toBeTruthy();
    });
    it('lineEllipseIntersects with rotate', () => {
        const rst = lineEllipseIntersects([0, -5], [0, 5], [0, 0], 3, 2, Math.PI / 2);
        expect(rst).toBeDefined();
        if (rst) {
    ...
    it('linePolygonIntersects', () => {
        const rst = linePolygonIntersects([5, 5], [15, 5], [
            [0, 0],
            [10, 0],
            [10, 10],
            [0, 10],
    ...
    it('linePolylineIntersects', () => {
        const rst = linePolylineIntersects([5, 5], [-5, 5], [
            [0, 0],
            [10, 0],
            [10, 10],
            [0, 10],
    ...
    it('rotatePoints', () => {
        ...
        expect(rst.every((p, i) => {
            return (almostEqual(p[0], expected[i][0]) && almostEqual(p[1], expected[i][1]));
        })).toBeTruthy();
    });
...
//# sourceMappingURL=math-utils.unit.spec.js.map