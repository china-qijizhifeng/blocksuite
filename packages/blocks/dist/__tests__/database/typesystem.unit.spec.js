...
import { tArray, tFunction, tTypeRef, tTypeVar, tUnion, tUnknown, typesystem, } from '../../database-block/data-view/logical/typesystem.js';
...
describe('subtyping', () => {
    test('all type is subtype of unknown', () => {
        expect(typesystem.isSubtype(tUnknown.create(), tBoolean.create())).toBe(true);
        expect(typesystem.isSubtype(tUnknown.create(), tString.create())).toBe(true);
        expect(typesystem.isSubtype(tUnknown.create(), tArray(tString.create()))).toBe(true);
        expect(typesystem.isSubtype(tUnknown.create(), tUnion([tString.create(), tBoolean.create()]))).toBe(true);
...
describe('function apply', () => {
    test('generic type function', () => {
        ...
        const instancedFn = typesystem.instance({}, [tBoolean.create()], tBoolean.create(), fn);
    ...
    test('tags infer', () => {
        ...
        const tags = [{ id: 'a', value: 'b', color: 'c' }];
        const instancedFn = typesystem.instance({}, [tTag.create({ tags })], tBoolean.create(), fn);
        const instancedFnArray = typesystem.instance({}, [tArray(tTag.create({ tags }))], tBoolean.create(), fnArray);
        expect(instancedFn.args[1]).toStrictEqual(tArray(tTag.create({ tags })));
        expect(instancedFnArray.args[1]).toStrictEqual(tArray(tTag.create({ tags })));
    });
});
//# sourceMappingURL=typesystem.unit.spec.js.map