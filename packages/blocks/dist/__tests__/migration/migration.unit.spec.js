...
describe('block migration', () => {
    test('update shape and text element, `isBold` -> `bold`, `isItalic` -> `italic`, surface v3 -> v4', async () => {
        ...
        let surfaceElements = doc
            ...
            .get('zUHGq4EHQJ')
            .get('prop:elements');
        let text = surfaceElements.get('Cj2MNGM9UK');
        let shape = surfaceElements.get('Rl2IT9rGoP');
        ...
        surfaceElements = doc
            ...
            .get('prop:elements')
            .get('value');
        text = surfaceElements.get('Cj2MNGM9UK');
        shape = surfaceElements.get('Rl2IT9rGoP');
    ...
    test('fix wrong connector data', async () => {
        ...
        const surfaceElements = doc
            ...
            .get('BOpLR3siGx')
            .get('prop:elements');
        let connector = surfaceElements.get('Gt8_2oZB8h');
    ...
    test('frame element to block', async () => {
        ...
        const pageChildren = page.get('sys:children');
    ...
    test('update database block title data', async () => {
        ...
        let databaseBlock = doc.getMap('blocks').get('Y76JkP9XRn').toJSON();
        expect(databaseBlock['prop:titleColumnName']).toBe('Title');
        expect(databaseBlock['prop:titleColumnWidth']).toBe(200);
        expect(databaseBlock['prop:columns'].find((v) => v.type === 'title')).toBeUndefined();
        ...
        databaseBlock = doc.getMap('blocks').get('Y76JkP9XRn').toJSON();
        expect(databaseBlock['prop:titleColumnName']).toBeUndefined();
        expect(databaseBlock['prop:titleColumnWidth']).toBeUndefined();
        const titleColumn = databaseBlock['prop:columns'].find((v) => v.type === 'title');
        expect(titleColumn.type).toBe('title');
        for (const view of databaseBlock['prop:views']) {
            if (view['mode'] === 'table') {
                expect(view['columns'].find((v) => v.id === titleColumn.id).width).toBe(200);
            }
        }
    });
});
//# sourceMappingURL=migration.unit.spec.js.map