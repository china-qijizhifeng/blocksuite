import { assertFlavours, asyncFocusRichText, asyncSetInlineRange, } from '../../_common/utils/index.js';
...
export const updateBlockType = (ctx, next) => {
    ...
    if (!hasSameDoc) {
        // doc check
        console.error('Not all models have the same doc instance, the result for update text type may not be correct', selectedBlocks);
    }
    const mergeToCode = (_, next) => {
    ...
    const focusText = (ctx, next) => {
        ...
        const allTextUpdated = updatedBlocks.map(model => onModelTextUpdated(host, model));
    ...
    const [result, resultCtx] = std.command
        ...
        // update block type
        .try(chain => [
        chain.inline(mergeToCode),
        chain.inline(appendDivider),
        chain.inline((_, next) => {
            const newModels = [];
