    ...
    constructor(host) {
        ...
        this._onArrowDown = (ctx) => {
            ...
            const [result] = this._std.command
                ...
                cmd
                    ...
                    if (!matchFlavours(nextBlock.model, [
                        'affine:paragraph',
                        'affine:list',
                        'affine:code',
                    ])) {
        ...
        this._onArrowUp = (ctx) => {
            ...
            const [result] = this._std.command
                ...
                cmd
                    ...
                    if (!matchFlavours(prevBlock.model, [
                        'affine:paragraph',
                        'affine:list',
                        'affine:code',
                    ])) {
        ...
        this._onSelectAll = ctx => {
            ...
            view.walkThrough(nodeView => {
                ...
                acc || cur.every(path => nodeView.path.includes(path)), false)) {
                    blocks.push(selection.create('block', {
        ...
        this._bindQuickActionHotKey = () => {
            quickActionConfig.forEach(config => {
                ...
                this.host.bindHotKey({
                    [config.hotkey]: ctx => {
                        if (!config.showWhen(this.host.host))
                            return;
                        ctx.get('defaultState').event.preventDefault();
                        ...
                        [key]: ctx => {
                            ...
                            const [result] = this._std.command
                                ...
                                onModelElementUpdated(host, codeModel, () => {
                                    const codeElement = this._std.view.viewFromPath('block', buildPath(codeModel));
...
//# sourceMappingURL=keymap-controller.js.map