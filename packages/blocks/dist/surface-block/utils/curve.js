...
function evaluate(v, t, type, normalized) {
    if (t == null || t < 0 || t > 1)
        return null;
    const x0 = v[0][0], y0 = v[0][1], x3 = v[3][0], y3 = v[3][1];
    let x1 = v[1][0], y1 = v[1][1], x2 = v[2][0], y2 = v[2][1];
    ...
    if (type === 0) {
        ...
        if (type === 3) {
            const x2 = 6 * ax * t + 2 * bx, y2 = 6 * ay * t + 2 * by, d = Math.pow(x * x + y * y, 3 / 2);
            x = d !== 0 ? (x * y2 - y * x2) / d : 0;
            y = 0;
        }
    }
...
export function getBezierPoint(values, t) {
    return evaluate(values, t, 0, false);
}
export function getBezierTangent(values, t) {
    return evaluate(values, t, 1, true);
}
export function getBezierNormal(values, t) {
    return evaluate(values, t, 2, true);
}
export function getBezierCurvature(values, t) {
    return evaluate(values, t, 3, false)?.[0];
}
export function getBezierNearestTime(values, point) {
    const count = 100;
...
export function getBezierNearestPoint(values, point) {
...
export function getBezierCurveBoundingBox(values) {
...
function align(points, [start, end]) {
    const tx = start[0], ty = start[1], a = -Math.atan2(end[1] - ty, end[0] - tx), d = function ([x, y]) {
        return [
...
function approximately(a, b, precision, epsilon = 0.000001) {
...
function roots(points, line) {
    ...
    if (order === 2) {
        const a = aligned[0][1], b = aligned[1][1], c = aligned[2][1], d = a - 2 * b + c;
        if (d !== 0) {
            const m1 = -Math.sqrt(b * b - a * c), m2 = -a + b, v1 = -(m1 + m2) / d, v2 = -(-m1 + m2) / d;
    ...
    let a = 3 * pa - 6 * pb + 3 * pc, b = -3 * pa + 3 * pb, c = pa;
    if (approximately(d, 0)) {
        ...
        // quadratic solution:
        const q = Math.sqrt(b * b - 4 * a * c), a2 = 2 * a;
    ...
    let u1, v1, x1, x2, x3;
    if (discriminant < 0) {
        const mp3 = -p / 3, mp33 = mp3 * mp3 * mp3, r = Math.sqrt(mp33), t = -q / (2 * r), cosphi = t < -1 ? -1 : t > 1 ? 1 : t, phi = Math.acos(cosphi), crtr = crt(r), t1 = 2 * crtr;
...
export function intersects(path, line) {
