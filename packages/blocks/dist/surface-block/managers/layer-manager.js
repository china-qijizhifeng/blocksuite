...
import { compare, getElementIndex, getLayerEndZIndex, insertToOrderedArray, isInRange, removeFromOrderedArray, renderableInEdgeless, ungroupIndex, updateLayersZIndex, } from './layer-utils.js';
export class LayerManager {
        ...
        if (batch === 'frame') {
            ...
            if (lastLayer.type === 'canvas') {
                ...
                const secondLastLayerIndex = secondLastLayer
                    ...
                    : null;
                return generateKeyBetween(secondLastLayerIndex, secondLastLayerIndex && secondLastLayerIndex >= lastLayer.indexes[0]
                    ? null
                    : lastLayer.indexes[0]);
            }
    ...
    static create(doc, surface) {
        const layerManager = new LayerManager(doc
            .getBlocks()
            .filter(model => model instanceof EdgelessBlockModel &&
            renderableInEdgeless(doc, surface, model)).concat(surface.elementModels));
