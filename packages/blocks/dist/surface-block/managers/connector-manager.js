...
import { ConnectorMode, isConnectorWithLabel, } from '../element-model/connector.js';
import { GroupElementModel } from '../element-model/group.js';
import { AStarRunner } from '../utils/a-star.js';
import { Bound, getBoundFromPoints } from '../utils/bound.js';
import { getBezierCurveBoundingBox, getBezierParameters, } from '../utils/curve.js';
import { almostEqual, clamp, getBoundsWithRotation, getPointFromBoundsWithRotation, isOverlap, isVecZero, lineIntersects, PI2, sign, toRadian, } from '../utils/math-utils.js';
...
export const ConnectorEndpointLocationsOnTriangle = [
    // At top
    [0.5, 0],
    // At right
    [0.75, 0.5],
...
export function calculateNearestLocation(point, bounds, locations = ConnectorEndpointLocations, shortestDistance = Number.POSITIVE_INFINITY) {
...
function rBound(ele, anti = false) {
...
export function getAnchors(ele) {
    const bound = Bound.deserialize(ele.xywh);
    const offset = 10;
    const anchors = [];
    ...
    [
        ...
        assertExists(rst);
        const originPoint = getPointFromBoundsWithRotation({ ...bound, rotate: -rotate }, rst[0]);
        anchors.push({ point: rst[0], coord: bound.toRelative(originPoint) });
    });
    return anchors;
}
function getConnectableRelativePosition(connectable, position) {
...
export function getNearestConnectableAnchor(ele, point) {
    const anchors = getAnchors(ele);
    return closestPoint(anchors.map(a => a.point), point);
}
...
function computePoints(startPoint, endPoint, nextStartPoint, lastEndPoint, startBound, endBound, expandStartBound, expandEndBound) {
    ...
    const result = getConnectablePoints(startPoint, endPoint, nextStartPoint, lastEndPoint, startBound, endBound, expandStartBound, expandEndBound);
    const points = result.points;
    nextStartPoint = result.nextStartPoint;
    lastEndPoint = result.lastEndPoint;
    const finalPoints = filterConnectablePoints(filterConnectablePoints(points, expandStartBound?.expand(-1) ?? null), expandEndBound?.expand(-1) ?? null);
    return [finalPoints, startPoint, endPoint, nextStartPoint, lastEndPoint];
}
function downscalePrecision(point) {
    return [
...
function filterConnectablePoints(points, bound) {
...
function pushWithPriority(points, vecs, priority = 0) {
    points.push(...vecs.map(vec => [...vec, priority]));
}
function pushLineIntersectsToPoints(points, aLine, bLine, priority = 0) {
    const rst = lineIntersects(aLine[0], aLine[1], bLine[0], bLine[1], true);
    if (rst) {
        pushWithPriority(points, [rst], priority);
    }
}
function pushOuterPoints(points, expandStartBound, expandEndBound, outerBound) {
...
function pushBoundMidPoint(points, bound1, bound2, expandBound1, expandBound2) {
    if (bound1.maxX < bound2.x) {
        ...
        [
            ...
            pushLineIntersectsToPoints(points, line, [
                [midX, 0],
                [midX, 1],
            ], index === 0 || index === 1 ? 6 : 3);
        });
...
function pushGapMidPoint(points, point, bound, bound2, expandBound, expandBound2) {
    /** on top or on bottom */
    if (almostEqual(point[1], bound.y, 0.02) ||
...
function removeDulicatePoints(points) {
...
function getConnectablePoints(startPoint, endPoint, nextStartPoint, lastEndPoint, startBound, endBound, expandStartBound, expandEndBound) {
    const lineBound = Bound.fromPoints([startPoint, endPoint]);
    const outerBound = expandStartBound &&
        expandEndBound &&
    ...
    const startEnds = [nextStartPoint, lastEndPoint].map(point => {
        return points.find(item => almostEqual(item[0], point[0], 0.02) &&
...
function getDirectPath(startPoint, endPoint) {
    if (almostEqual(startPoint[0], endPoint[0], 0.02) ||
...
function mergePath(points) {
    ...
    for (let i = 1; i < points.length - 1; i++) {
        ...
        if (almostEqual(last[0], cur[0], 0.02) &&
            almostEqual(cur[0], next[0], 0.02))
            continue;
        if (almostEqual(last[1], cur[1], 0.02) &&
    ...
    for (let i = 0; i < result.length - 1; i++) {
        ...
        try {
            assertEquals(almostEqual(cur[0], next[0], 0.02) ||
                almostEqual(cur[1], next[1], 0.02), true);
        }
        catch (_) {
            console.log(points);
            console.log(result);
...
function computeOffset(startBound, endBound) {
    ...
    if (overlap && startBound.upperLine[0][1] > endBound.lowerLine[0][1]) {
        dist = Vec.distanceToLineSegment(startBound.upperLine[0], startBound.upperLine[1], endBound.lowerLine[0], false);
    ...
    if (overlap && startBound.rightLine[0][0] < endBound.leftLine[0][0]) {
        dist = Vec.distanceToLineSegment(startBound.rightLine[0], startBound.rightLine[1], endBound.leftLine[0], false);
    ...
    if (overlap && startBound.lowerLine[0][1] < endBound.upperLine[0][1]) {
        dist = Vec.distanceToLineSegment(startBound.lowerLine[0], startBound.lowerLine[1], endBound.upperLine[0], false);
...
function getNextPoint(bound, point, offsetX = 10, offsetY = 10, offsetW = 10, offsetH = 10) {
    const result = Array.from(point);
    if (almostEqual(bound.x, result[0]))
        result[0] -= offsetX;
    else if (almostEqual(bound.y, result[1]))
        result[1] -= offsetY;
    else if (almostEqual(bound.maxX, result[0]))
        result[0] += offsetW;
    else if (almostEqual(bound.maxY, result[1]))
        result[1] += offsetH;
    else {
        ...
        const slope = Math.abs(point.tangent[0]) < Math.abs(point.tangent[1]) ? 0 : 1;
        // if the slope is big, use the x direction
        if (slope === 0) {
            if (xDirection > 0) {
                const intersects = lineIntersects(bound.rightLine[0], bound.rightLine[1], result, [bound.maxX + 10, result[1]]);
                assertExists(intersects);
                result[0] = intersects[0] + offsetX;
            }
            else {
                const intersects = lineIntersects(bound.leftLine[0], bound.leftLine[1], result, [bound.x - 10, result[1]]);
                assertExists(intersects);
                result[0] = intersects[0] - offsetX;
            }
        }
            ...
            if (yDirection > 0) {
                const intersects = lineIntersects(bound.lowerLine[0], bound.lowerLine[1], result, [result[0], bound.maxY + 10]);
                assertExists(intersects);
                result[1] = intersects[1] + offsetY;
            }
            else {
                const intersects = lineIntersects(bound.upperLine[0], bound.upperLine[1], result, [result[0], bound.y - 10]);
                assertExists(intersects);
                result[1] = intersects[1] - offsetY;
            }
        }
...
function computeNextStartEndpoint(startPoint, endPoint, startBound, endBound, startOffset, endOffset) {
    const nextStartPoint = startBound && startOffset
        ? getNextPoint(startBound, startPoint, startOffset[0], startOffset[1], startOffset[2], startOffset[3])
        : startPoint;
    const lastEndPoint = endBound && endOffset
        ? getNextPoint(endBound, endPoint, endOffset[0], endOffset[1], endOffset[2], endOffset[3])
        : endPoint;
    return [nextStartPoint, lastEndPoint];
}
function adjustStartEndPoint(startPoint, endPoint, startBound = null, endBound = null) {
    if (!endBound) {
...
function renderRect(ctx, bounds, color, lineWidth) {
    const { x, y, w, h } = bounds;
    ctx.save();
    ctx.beginPath();
...
export class ConnectionOverlay extends Overlay {
    ...
    render(ctx) {
        ...
        const radius = 5 / zoom;
        const color = getComputedStyle(this._service.host).getPropertyValue('--affine-text-emphasis-color');
        ctx.globalAlpha = 0.6;
     ...
     */
    renderConnector(point, excludedIds = []) {
        ...
        for (let i = 0; i < connectables.length; i++) {
            ...
            if (excludedIds.includes(connectable.id))
                continue;
            // then check if in expanded bound
            const bound = Bound.deserialize(connectable.xywh);
            const rotateBound = Bound.from(getBoundsWithRotation(rBound(connectable)));
            // FIXME: the real path needs to be expanded: diamod, ellipse, trangle.
            if (!rotateBound.expand(10).isPointInBound(point))
                continue;
            ...
            for (; j < len; j++) {
                ...
                const anchorViewCoord = service.viewport.toViewCoord(anchor.point[0], anchor.point[1]);
            ...
            if (Vec.dist(nearestPoint, point) < 8) {
                ...
                const originPoint = getPointFromBoundsWithRotation(rBound(connectable, true), nearestPoint);
            ...
            if (connectable.hitTest(point[0], point[1], {
...
export class ConnectorPathGenerator {
    constructor(options) {
        ...
        this._aStarRunner = null;
    }
    _getConnectorEndElement(connector, type) {
        const id = connector[type].id;
        if (id) {
            return this.options.getElementById(id);
        }
        return null;
    }
    _getConnectionPoint(connector, type) {
    ...
    _generateStraightConnectorPath(connector) {
        const { source, target } = connector;
    ...
    _computeStartEndPoint(connector) {
        const { source, target } = connector;
        const start = this._getConnectorEndElement(connector, 'source');
        const end = this._getConnectorEndElement(connector, 'target');
    ...
    _generateCurveConnectorPath(connector) {
        const { source, target } = connector;
        if (source.id || target.id) {
            let startPoint;
            let endPoint;
            ...
            if (target.id) {
                ...
                endPoint.in = Vec.mul(endTangentVertical, Math.max(100, Math.abs(Vec.pry(Vec.sub(startPoint, endPoint), endTangentVertical)) / 3));
            }
            return [startPoint, endPoint];
        }
        else {
    ...
    _prepareOrthogonalConnectorInfo(connectorInfo) {
        ...
        const [nextStartPoint, lastEndPoint] = computeNextStartEndpoint(startPoint, endPoint, startBound, endBound, startOffset, endOffset);
    ...
    _generateConnectorPath(connector) {
    ...
    updatePath(connector, path) {
        const points = path ?? this._generateConnectorPath(connector) ?? [];
        const bound = connector.mode === ConnectorMode.Curve
    ...
    generateOrthogonalConnectorPath(input) {
        ...
        const points = computePoints(startPoint, endPoint, nextStartPoint, lastEndPoint, startBound, endBound, expandStartBound, expandEndBound);
        ...
        this._aStarRunner = new AStarRunner(finalPoints, nextStartPoint, lastEndPoint, startPoint, endPoint, blocks, expandBlocks);
    ...
    hasRelatedElement(connecter) {
        const { source, target } = connecter;
        if ((source.id && !this.options.getElementById(source.id)) ||
