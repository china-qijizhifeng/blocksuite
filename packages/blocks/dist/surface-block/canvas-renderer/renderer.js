...
export class Renderer extends Viewport {
    ...
    _renderByBound(ctx, matrix, rc, bound, surfaceElements, overLay = false) {
        if (!ctx)
            return;
        const elements = surfaceElements ?? this.layerManager.canvasGrid.search(bound);
        for (const element of elements) {
            ctx.save();
            const display = element.display ?? true;
            if (display && intersects(getBoundsWithRotation(element), bound)) {
                const renderFn = modelRenderer[element.type];
                if (!renderFn) {
    ...
    getCanvasByBound(bound = this.viewportBounds, surfaceElements, canvas, clearBeforeDrawing, withZoom) {
