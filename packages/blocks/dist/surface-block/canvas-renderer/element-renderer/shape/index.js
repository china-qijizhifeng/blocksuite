...
export function shape(model, ctx, matrix, renderer, rc) {
    shapeRenderers[model.shapeType](model, ctx, matrix, renderer, rc);
    if (model.textDisplay) {
        renderText(model, ctx, renderer);
    }
}
function renderText(model, ctx, renderer) {
    const { x, y, text, color, fontSize, fontFamily, fontWeight, textAlign, w, h, textVerticalAlign, padding, } = model;
    if (!text)
        return;
    const [verticalPadding, horPadding] = padding;
    const font = getFontString(model);
    const { lineGap, lineHeight } = measureTextInDOM(fontFamily, fontSize, fontWeight);
    const metrics = getFontMetrics(fontFamily, fontSize, fontWeight);
    const lines = deltaInsertsToChunks(wrapTextDeltas(text, font, w - horPadding * 2));
    const horOffset = horizontalOffset(model.w, model.textAlign, horPadding);
    const vertOffset = verticalOffset(lines, lineHeight + lineGap, h, textVerticalAlign, verticalPadding) +
    ...
    for (const [lineIndex, line] of lines.entries()) {
        for (const delta of line) {
            ...
            horOffset + 0.5, lineIndex * lineHeight + vertOffset);
    ...
    const bound = new Bound(x + (w - maxLineWidth) / 2, y + vertOffset - 2, maxLineWidth, lineHeight * lines.length);
