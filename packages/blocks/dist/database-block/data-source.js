...
import { databaseBlockAllColumnMap, databaseBlockColumns, } from './columns/index.js';
import { HostContextKey } from './context/host-context.js';
import { BaseDataSource, columnPresets, createUniComponentFromWebComponent, insertPositionToIndex, } from './data-view/index.js';
...
export class DatabaseBlockDataSource extends BaseDataSource {
    ...
    propertyAdd(insertToPosition, type) {
        this.doc.captureSync();
        return this._model.addColumn(insertToPosition, databaseBlockAllColumnMap[type ?? columnPresets.multiSelectColumnConfig.type].model.create(this.newColumnName()));
    }
    ...
    onCellUpdate(rowId, propertyId, callback) {
        let lastDisposable;
        let lastValue = this.cellGetValue(rowId, propertyId);
        const cb = () => {
            ...
            if (!onUpdate) {
                ...
                if (value != lastValue) {
                    ...
                    lastDisposable =
                        value != null
                            ? onUpdate(value, this.propertyGetData(propertyId), callback)
                            : undefined;
                }
            }
            lastValue = value;
