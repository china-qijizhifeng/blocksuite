...
export class KanbanSelectionController {
    ...
    constructor(host) {
        ...
        this.shiftClickCard = (event) => {
            ...
            const newCards = cards.findIndex(card => card.cardId === closestCardId) >= 0
    ...
    hostConnected() {
        this.host.disposables.add(this.host.selectionUpdated.on(selection => {
    ...
    getNextFocusCell(selection, index, nextPosition) {
        ...
        throw new Error('Unknown arrow keys, only support: up, down, left, and right keys.');
    }
    ...
    moveCard(rowId, key) {
        ...
        this.view.groupHelper?.moveCardTo(rowId, selection.cards[0].groupKey, key, 'start');
        requestAnimationFrame(() => {
