...
export class TableClipboardController {
    ...
    constructor(host) {
        this.host = host;
        this._onCopy = (_context, tableSelection, isCut = false) => {
            const view = this.host;
            const data = this.host.view;
            // cells
            // For database paste inside.
            const copiedValues = getCopiedValuesFromSelection(tableSelection, data, view);
        ...
        this._onCut = (_context, tableSelection) => {
        ...
        this._onPaste = async (_context) => {
            ...
            if (tableSelection) {
                ...
                if (!copiedValues)
                    return;
                const copyedSelectionData = JSON.parse(copiedValues);
                // paste cells
                ...
                pasteToCells(data, view, copyedSelectionData, tableSelection.groupKey, rowStartIndex, columnStartIndex, rowLength, columnLength);
            }
            return true;
        };
    ...
    hostConnected() {
        ...
        this.host.disposables.add(this.host.handleEvent('cut', ctx => {
            const tableSelection = this.host.selectionController.selection;
            if (!tableSelection)
                return false;
...
function copyCellsValue(selection, data, view, isCut = false) {
    const { rowsSelection, columnsSelection, focus, groupKey } = selection;
    const values = [];
    if (rowsSelection && !columnsSelection) {
        // rows
        const { start, end } = rowsSelection;
        const titleIndex = data.columnsWithoutFilter.findIndex(id => data.columnGetType(id) === 'title');
        for (let i = start; i <= end; i++) {
            const container = view.selectionController.getCellContainer(groupKey, i, titleIndex);
            const value = (cellToStringMap['title']?.(container) ?? '');
            values.push([value]);
        }
    }
        ...
        for (let i = rowsSelection.start; i <= rowsSelection.end; i++) {
            ...
            for (let j = columnsSelection.start; j <= columnsSelection.end; j++) {
                ...
                const container = view.selectionController.getCellContainer(groupKey, i, j);
                ...
                if (isCut && !UNSUPPORTED_COLUMNS.includes(column.type)) {
                    ...
                    if (column.type === 'title' || column.type === 'rich-text') {
                        const yText = container?.column.getValue(container.rowId);
        ...
        const container = view.selectionController.getCellContainer(groupKey, focus.rowIndex, focus.columnIndex);
...
function getCopiedValuesFromSelection(selection, data, view) {
    const { rowsSelection, columnsSelection, focus, groupKey } = selection;
    const values = [];
    if (rowsSelection && !columnsSelection) {
        // rows
...
function pasteToCells(data, view, copied, groupKey, rowStartIndex, columnStartIndex, rowLength, columnLength) {
    ...
    for (let i = 0; i < rowLength; i++) {
        for (let j = 0; j < columnLength; j++) {
            ...
            const targetContainer = view.selectionController.getCellContainer(groupKey, rowIndex, columnIndex);
            const rowId = targetContainer?.dataset.rowId;
            const columnId = targetContainer?.dataset.columnId;
            if (rowId && columnId) {
                const value = targetContainer?.column.setValueFromString(srcColumn.value);
                data.cellUpdateValue(rowId, columnId, value);
            }
        }
    }
}
const cellToStringMap = {
    'rich-text': container => {
        const cell = container?.querySelector('affine-database-rich-text-cell-editing');
