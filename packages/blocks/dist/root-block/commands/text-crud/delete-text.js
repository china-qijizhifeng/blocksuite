...
export const deleteTextCommand = (ctx, next) => {
    ...
    assertExists(textSelection, '`textSelection` is required, you need to pass it in args or use `getTextSelection` command before adding this command to the pipeline.');
    const host = ctx.std.host;
    assertExists(host.rangeManager);
    const range = host.rangeManager.textSelectionToRange(textSelection);
    if (!range)
        return;
    const selectedElements = host.rangeManager.getSelectedBlockElementsByRange(range, {
        mode: 'flat',
    });
...
//# sourceMappingURL=delete-text.js.map