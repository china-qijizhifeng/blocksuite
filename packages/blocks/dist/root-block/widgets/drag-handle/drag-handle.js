...
import { getBlockElementsExcludeSubtrees, getCurrentNativeRange, getModelByBlockComponent, isInsideEdgelessEditor, isInsidePageEditor, matchFlavours, Point, Rect, } from '../../../_common/utils/index.js';
...
let AffineDragHandleWidget = (() => {
    ...
    var AffineDragHandleWidget = class extends _classSuper {
        ...
        constructor() {
            ...
            this._lastHoveredBlockPath = (__runInitializers(this, __dragHoverRect_extraInitializers), null);
            ...
            this._throttledPointerMoveHandler = throttle(ctx => {
                ...
                const closestNoteBlock = getClosestNoteBlock(this.host, this.rootElement, point);
            ...
            this._getDropResult = (state) => {
                const point = new Point(state.raw.x, state.raw.y);
                const closestBlockElement = getClosestBlockByPoint(this.host, this.rootElement, point);
                ...
                if (containBlock(this.draggingElements.map(blockElement => blockElement.model.id), blockId) ||
                    containChildBlock(this.draggingElements, blockPath)) {
                    return null;
                }
                let rect = null;
                let dropType = 'before';
                const result = calcDropTarget(point, model, closestBlockElement, this.draggingElements, this.scale * this.cumulativeParentScale, isDraggedElementNote === false);
                if (result) {
                    rect = result.rect;
            ...
            this._createDragPreview = (blockElements, state, dragPreviewEl, dragPreviewOffset) => {
                let dragPreview;
                if (dragPreviewEl) {
                    ...
                    const selectedIds = blockElements.map(blockElement => blockElement.model.id);
            ...
            this._startDragging = (blockElements, state, dragPreviewEl, dragPreviewOffset) => {
                ...
                this.dragPreview = this._createDragPreview(blockElements, state, dragPreviewEl, dragPreviewOffset);
                this.dragging = true;
                this._changeCursorToGrabbing();
                this._createDropIndicator();
                this._hide();
            ...
            this._getBlockElementFromViewStore = (path) => {
                return this.host.view.getBlock(path);
            };
            // Need to consider block padding and scale
            this._getTopWithBlockElement = (blockElement) => {
            ...
            this._showDragHandleOnTopLevelBlocks = async () => {
                ...
                if (!this._anchorBlockPath)
                    return;
                const blockElement = this.anchorBlockElement;
                assertExists(blockElement);
                const edgelessElement = edgelessRoot.service.getElementById(blockElement.model.id);
            ...
            this._getHoveredBlocks = () => {
                ...
                if (selections.length > 0 && includeTextSelection(selections)) {
                    ...
                    if (!range)
                        return [];
                    blockElements = this._rangeManager.getSelectedBlockElementsByRange(range, {
                ...
                if (containBlock(blockElements.map(block => PathFinder.id(block.path)), this._anchorBlockPath)) {
                    return blockElements;
                }
                return [hoverBlock];
            };
            this._getDraggingAreaRect = (blockElement) => {
            ...
            this._setSelectedBlocks = (blockElements, noteId) => {
                const { selection } = this.host;
                const selections = blockElements.map(blockElement => selection.create('block', {
                ...
                if (isInsideEdgelessEditor(this.host)) {
                    ...
                    const surfaceSelection = selection.create('surface', blockElements[0].blockId, [surfaceElementId], true);
            ...
            this._clickHandler = ctx => {
                ...
                if (!insideDragHandle)
                    return;
                if (!this._anchorBlockId || !this._anchorBlockPath)
                    return;
                const { selection } = this.host;
            ...
            this._onDragStart = (state) => {
                ...
                if (selections.length > 0 && includeTextSelection(selections)) {
                    ...
                    if (nativeSelection && nativeSelection.rangeCount > 0) {
                        ...
                        const blockElements = this._rangeManager.getSelectedBlockElementsByRange(range, {
                            match: el => el.model.role === 'content',
                            mode: 'highest',
                        });
                ...
                const blockElementsExcludingChildren = getBlockElementsExcludeSubtrees(blockElements);
                if (blockElementsExcludingChildren.length === 0)
                    return false;
        ...
        firstUpdated() {
            ...
            if (isInsidePageEditor(this.host)) {
                ...
                const pageRoot = this.rootElement;
