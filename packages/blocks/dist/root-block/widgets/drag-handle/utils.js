...
import { findClosestBlockElement, getClosestBlockElementByElement, getClosestBlockElementByPoint, getDropRectByPoint, getHoveringNote, getRectByBlockElement, isInsidePageEditor, matchFlavours, Point, Rect, } from '../../../_common/utils/index.js';
import { Bound } from '../../../surface-block/index.js';
import { isEmbedSyncedDocBlock } from '../../edgeless/utils/query.js';
import { BLOCK_CHILDREN_CONTAINER_PADDING_LEFT, DRAG_HANDLE_CONTAINER_HEIGHT, DRAG_HANDLE_CONTAINER_OFFSET_LEFT, DRAG_HANDLE_CONTAINER_OFFSET_LEFT_LIST, EDGELESS_NOTE_EXTRA_PADDING, NOTE_CONTAINER_PADDING, } from './config.js';
const heightMap = {
    text: 23,
    h1: 40,
    h2: 36,
...
export const containChildBlock = (blockElements, childPath) => {
...
export const getNoteId = (blockElement) => {
 ...
 */
export const isBlockPathEqual = (path1, path2) => {
    if (!path1 || !path2) {
        return false;
    }
    return path1 === path2;
};
export const getContainerOffsetPoint = (state) => {
...
export const getClosestNoteBlock = (editorHost, rootElement, point) => {
...
export const getClosestBlockByPoint = (editorHost, rootElement, point) => {
    ...
    const closestBlockElement = (blockElement &&
        PathFinder.includes(blockElement.path, closestNoteBlock.path)
        ? blockElement
        : findClosestBlockElement(closestNoteBlock, point.clone(), blockSelector));
...
allowSublist = true) {
    ...
    if (type === 'before') {
        ...
        const hasChild = element.childBlockElements.length;
        if (allowSublist &&
...
export const getDropResult = (event, scale = 1) => {
    ...
    const closestBlockElement = getClosestBlockElementByPoint(point);
    if (!closestBlockElement) {
...
export function getDragHandleLeftPadding(blockElements) {
    const hasToggleList = blockElements.some(blockElement => matchFlavours(blockElement.model, ['affine:list']) &&
...
let previousEle = [];
export function updateDragHandleClassName(blockElements = []) {
