...
const ParagraphPanel = ({ formatBar, host, ref: containerRef, }) => {
    const config = formatBar.configItems
        .filter((item) => item.type === 'paragraph-action')
        .filter(({ flavour }) => host.doc.schema.flavourSchemaMap.has(flavour));
    const renderedConfig = repeat(config, item => html `<icon-button
        width="100%"
        height="32px"
...
export const ParagraphButton = (formatBar) => {
    ...
    const paragraphIcon = selectedBlockElements.length < 1
        ? textConversionConfigs[0].icon
        : textConversionConfigs.find(({ flavour, type }) => selectedBlockElements[0].flavour === flavour &&
            selectedBlockElements[0].model.type ===
                type)?.icon ?? textConversionConfigs[0].icon;
    ...
    const { setFloating, setReference } = whenHover(isHover => {
        if (!isHover) {
            const panel = formatBar.shadowRoot?.querySelector('.paragraph-panel');
            if (!panel)
                return;
        ...
        const button = formatBar.shadowRoot?.querySelector('.paragraph-button');
        const panel = formatBar.shadowRoot?.querySelector('.paragraph-panel');
        assertExists(button);
        assertExists(panel);
...
//# sourceMappingURL=paragraph-button.js.map