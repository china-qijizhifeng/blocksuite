...
import { getActiveConnectorStrokeColor, getActiveShapeColor, setEdgelessToolAction, updateShapeOverlay, } from './utils.js';
//----------------------------------------------------------
...
const pie = new PieMenuBuilder({
    ...
    trigger: ({ keyEvent: ev, rootElement }) => {
        if (isControlledKeyboardEvent(ev))
            return false;
...
const shapes = [
    {
        type: ShapeType.Rect,
        label: 'Rect',
        icon: (style) => style === ShapeStyle.General ? SquareIcon : ScribbledSquareIcon,
    },
    {
        type: ShapeType.Ellipse,
        label: 'Ellipse',
        icon: (style) => style === ShapeStyle.General ? EllipseIcon : ScribbledEllipseIcon,
    },
    {
        type: ShapeType.Triangle,
        label: 'Triangle',
        icon: (style) => style === ShapeStyle.General ? TriangleIcon : ScribbledTriangleIcon,
    },
    {
        type: ShapeType.Diamond,
        label: 'Diamond',
        icon: (style) => style === ShapeStyle.General ? DiamondIcon : ScribbledDiamondIcon,
    },
];
shapes.forEach(shape => {
    pie.command({
        label: shape.label,
        icon: ({ rootElement }) => {
            const attributes = rootElement.service.editPropsStore.getLastProps('shape');
...
pie.command({
    label: 'Toggle Style',
    icon: ({ rootElement }) => {
        const { shapeStyle } = rootElement.service.editPropsStore.getLastProps('shape');
    ...
    action: ({ rootElement }) => {
        const { shapeStyle } = rootElement.service.editPropsStore.getLastProps('shape');
        const toggleType = shapeStyle === ShapeStyle.General
