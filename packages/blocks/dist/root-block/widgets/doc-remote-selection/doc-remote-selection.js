  ...
  `; }
        ...
        _getCursorRect(selections) {
            ...
            if (textSelection) {
                const rangeManager = this.host.rangeManager;
                assertExists(rangeManager);
                const range = rangeManager.textSelectionToRange(this._selectionManager.create('text', {
                    from: {
        ...
        connectedCallback() {
            ...
            this.disposables.add(this._selectionManager.slots.remoteChanged.on(throttle((remoteSelections) => {
                const status = this.doc.awarenessStore.getStates();
                this._remoteSelections = [...remoteSelections.entries()].map(([id, selections]) => {
                    return {
                        id,
                        selections,
        ...
        render() {
            ...
            const selections = this._remoteSelections.flatMap(({ selections, id, user }) => {
