...
import { asyncFocusRichText, buildPath, focusTitle, getDocTitleInlineEditor, matchFlavours, NoteDisplayMode, } from '../../_common/utils/index.js';
...
let PageRootBlockComponent = (() => {
    let _classDecorators = [customElement('affine-page-root')];
    ...
    var PageRootBlockComponent = class extends _classSuper {
        ...
        constructor() {
            ...
            this.prependParagraphWithText = (__runInitializers(this, _rootElementContainer_extraInitializers), (text) => {
                const newFirstParagraphId = this.doc.addBlock('affine:paragraph', { text }, this._getDefaultNoteBlock(), 0);
            ...
            this.focusFirstParagraph = () => {
                ...
                const firstText = defaultNote?.children.find(block => matchFlavours(block, ['affine:paragraph', 'affine:list', 'affine:code']));
                if (firstText) {
        ...
        static {
            ...
            PageRootBlockComponent = _classThis = _classDescriptor.value;
  ...
  `; }
        ...
        _getDefaultNoteBlock() {
            return (this.doc.root?.children.find(child => child.flavour === 'affine:note') ??
        ...
        firstUpdated() {
            ...
            const noteModels = this.model.children.filter(model => matchFlavours(model, ['affine:note']));
            ...
            this.handleEvent('click', ctx => {
                ...
                const lastNote = this.model.children
                    ...
                    if (!isNote)
                        return false;
                    const note = child;
                    const displayOnDoc = !!note.displayMode &&
        ...
        renderBlock() {
            const content = html `${repeat(this.model.children.filter(child => {
                const isNote = matchFlavours(child, ['affine:note']);
                const note = child;
                const displayOnEdgeless = !!note.displayMode &&
                    note.displayMode === NoteDisplayMode.EdgelessOnly;
    ...
    return PageRootBlockComponent = _classThis;
})();
