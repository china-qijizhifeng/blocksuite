...
import { Bound, getQuadBoundsWithRotation, rotatePoints, } from '../../../../surface-block/index.js';
...
export class HandleResizeManager {
    ...
    constructor(onDragStart, onResizeMove, onRotateMove, onDragEnd) {
        ...
        this._aspectRatio = 1;
        this._locked = false;
        this._proportion = false;
        this._shiftKey = false;
        this._proportional = false;
        this._rotation = false;
        this._target = null;
        this._dragging = false;
        this.onPointerDown = (e, direction, proportional = false) => {
            ...
            this._dragging = true;
            this._onDragStart();
            const _onPointerMove = ({ x, y, shiftKey }) => {
    ...
    _onResize(proportion) {
        const { _aspectRatio, _dragDirection, _dragPos, _rotate, _resizeMode, _zoom, _target, _originalRect, _currentRect, } = this;
        ...
        const { start: { x: startX, y: startY }, end: { x: endX, y: endY }, } = _dragPos;
        ...
        if (isCorner || isAll || isEdgeAndCorner) {
            ...
            dp.x += deltaX;
            dp.y += deltaY;
            if (_dragDirection === HandleDirection.Left ||
                ...
                const [[x3, y3]] = rotatePoints([[point.x, point.y]], coorPoint, _rotate);
                dp.x = x3;
                dp.y = y3;
            }
            ...
            const isDraggingCorner = _dragDirection === HandleDirection.TopLeft ||
        ...
        if (isCorner || isAll || isEdgeAndCorner) {
            if (this._bounds.size === 1) {
                ...
                process = ({ bound: { x, y, w, h }, path }, id) => {
                    ...
                    newBounds.set(id, {
                        bound: new Bound(center.x - newWidth / 2, center.y - newHeight / 2, newWidth, newHeight),
                        matrix: m2,
                        path,
                    });
                };
            ...
            // include notes, <---->
            const m2 = new DOMMatrix().scaleSelf(scale.x, scale.y, 1, fixedPoint.x, fixedPoint.y, 0);
            process = ({ bound: { x, y, w, h }, rotate = 0, path }, id) => {
                ...
                if (rotate) {
                    ...
                    if (_currentRect.width <= width) {
                        ...
                        center.x = Math.max(_currentRect.left + hrw, Math.min(lx, _currentRect.left + _currentRect.width - hrw));
                        newWidth = w;
                        newHeight = h;
                    }
                }
    ...
    _onRotate(shiftKey = false) {
        const { _originalRect: { left: minX, top: minY, right: maxX, bottom: maxY }, _dragPos: { start: { x: startX, y: startY }, end: { x: endX, y: endY }, }, _origin: { x: centerX, y: centerY }, _rotate, } = this;
        ...
        let y = endY;
        if (shiftKey) {
            const point = new DOMPoint(startX, startY).matrixTransform(new DOMMatrix()
        ...
        { x: (minX + maxX) / 2, y: (minY + maxY) / 2 }, delta);
        this._dragPos.start = { x, y };
        this._rotate += delta;
    }
    updateState(resizeMode, rotate, zoom, position, originalRect, proportion = false) {
