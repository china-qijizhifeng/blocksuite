...
import { Bound, ConnectorElementModel, ConnectorMode, GroupElementModel, ShapeType, } from '../../surface-block/index.js';
...
import { DEFAULT_NOTE_CHILD_FLAVOUR, DEFAULT_NOTE_CHILD_TYPE, DEFAULT_NOTE_TIP, } from './utils/consts.js';
...
import { mountConnectorLabelEditor, mountShapeTextEditor, } from './utils/text.js';
export class EdgelessPageKeyboardManager extends PageKeyboardManager {
    constructor(rootElement) {
        ...
        this.rootElement.bindHotKey({
            ...
            k: () => {
                if (this.rootElement.service.locked)
                    return;
                ...
                if (selection.selectedElements.length === 1 &&
                    ...
                    matchFlavours(selection.firstElement, [
                        'affine:note',
                    ])) {
            ...
            f: () => {
                if (this.rootElement.service.locked)
                    return;
                if (this.rootElement.service.selection.selectedElements.length !== 0 &&
            ...
            '-': () => {
                ...
                if (!rootElement.service.selection.editing &&
                    elements.length === 1 &&
            ...
            '@': () => {
                ...
                const { insertedLinkType } = std.command.exec('insertLinkByQuickSearch');
                insertedLinkType
                    ?.then(type => {
                    if (type) {
                        rootElement.service.telemetryService?.track('CanvasElementAdded', {
            ...
            'Shift-s': () => {
                ...
                const nextShapeType = getNextShapeType(attr.radius > 0 && attr.shapeType === ShapeType.Rect
                    ? 'roundedRect'
                    : attr.shapeType);
                this._setEdgelessTool(rootElement, {
                    type: 'shape',
                    shapeType: nextShapeType === 'roundedRect' ? ShapeType.Rect : nextShapeType,
                });
            ...
            'Mod-g': ctx => {
                if (this.rootElement.service.locked)
                    return;
                if (this.rootElement.service.selection.selectedElements.length > 1 &&
                    !this.rootElement.service.selection.editing) {
            ...
            Escape: () => {
                ...
                if (currentController instanceof LassoToolController &&
            ...
            Enter: () => {
                ...
                const target = service.getElementById(id);
                requestAnimationFrame(() => {
                    ...
                    if (isElementOutsideViewport(service.viewport, target, [20, 20])) {
                        ...
                        service.viewport.smoothTranslate(elementBound.x + elementBound.w / 2, elementBound.y + elementBound.h / 2);
                    }
                });
            },
            Tab: () => {
    ...
    _bindShiftKey() {
        this.rootElement.handleEvent('keyDown', ctx => {
            const event = ctx.get('defaultState').event;
            if (event instanceof KeyboardEvent) {
                this._shift(event);
            }
        }, { global: true });
        this.rootElement.handleEvent('keyUp', ctx => {
            const event = ctx.get('defaultState').event;
            if (event instanceof KeyboardEvent) {
                this._shift(event);
            }
    ...
    _space(event) {
      ...
      */
        ...
        if (isKeyDown) {
            ...
            edgeless.dispatcher.disposables.addFromEvent(document, 'keyup', revertToPrevTool);
        }
    }
    _shift(event) {
        const edgeless = this.rootElement;
        if (event.repeat)
            return;
        const shiftKeyPressed = event.key.toLowerCase() === 'shift' && event.shiftKey;
        if (shiftKeyPressed) {
    ...
    _delete() {
        const edgeless = this.rootElement;
        if (edgeless.service.locked)
            return;
        if (edgeless.service.selection.editing) {
            return;
        }
        deleteElements(edgeless.surface, edgeless.service.selection.selectedElements);
        edgeless.service.selection.clear();
        edgeless.service.selection.set(edgeless.service.selection.surfaceSelections);
    }
    _setEdgelessTool(edgeless, edgelessTool, ignoreActiveState = false) {
    ...
    _move(key, shift = false) {
        ...
        const mindmapNodes = selectedElements.filter(el => el.group instanceof MindmapElementModel);
        if (mindmapNodes.length > 0) {
            const node = mindmapNodes[0];
            const mindmap = node.group;
            ...
            switch (key) {
                ...
                case 'ArrowDown':
                    targetNode =
                        mindmap.getSiblingNode(node.id, key === 'ArrowDown' ? 'next' : 'prev', nodeDirection === LayoutType.RIGHT
                            ? 'right'
            ...
            if (targetNode) {
                ...
                if (isElementOutsideViewport(edgeless.service.viewport, targetNode, [90, 20])) {
                    const [dx, dy] = getNearestTranslation(edgeless.service.viewport, targetNode, [100, 20]);
                    edgeless.service.viewport.smoothTranslate(edgeless.service.viewport.centerX - dx, edgeless.service.viewport.centerY + dy);
                }
            }
            return;
        }
...
//# sourceMappingURL=edgeless-keyboard.js.map