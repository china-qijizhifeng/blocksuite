...
import { Bound, deserializeXYWH, getBoundsWithRotation, Overlay, } from '../../../surface-block/index.js';
...
export class EdgelessSnapManager extends Overlay {
    ...
    _alignDistributeHorizontally(rst, bound, threshold, viewport) {
        ...
        for (let i = 0; i < wBoxes.length; i++) {
            for (let j = i + 1; j < wBoxes.length; j++) {
                ...
                if (!lb.isHorizontalCross(rb))
                    continue;
                if (lb.isIntersectWithBound(rb))
                    continue;
                if (rb.maxX < lb.minX) {
                    const temp = rb;
                    rb = lb;
                ...
                const updateDif = () => {
                    ...
                    if (dif <= threshold && dif < min) {
                         ...
                         */
                        const ys = [lb.minY, lb.maxY, rb.minY, rb.maxY].sort((a, b) => a - b);
                        const y = (ys[1] + ys[2]) / 2;
    ...
    _alignDistributeVertically(rst, bound, threshold, viewport) {
        ...
        for (let i = 0; i < hBoxes.length; i++) {
            for (let j = i + 1; j < hBoxes.length; j++) {
                ...
                if (!ub.isVerticalCross(db))
                    continue;
                if (ub.isIntersectWithBound(db))
                    continue;
                if (db.maxY < ub.minX) {
                    const temp = ub;
                    ub = db;
                ...
                const updateDiff = () => {
                    ...
                    if (dif <= threshold && dif < min) {
                         ...
                         * calculate points to draw
                         */
                        const xs = [ub.minX, ub.maxX, db.minX, db.maxX].sort((a, b) => a - b);
                        const x = (xs[1] + xs[2]) / 2;
                        const offset = 2 / viewport.zoom;
                        const ys = [
    ...
    setupAlignables(alignables) {
        ...
        [
            ...
            if (viewportBounds.isOverlapWithBound(bounds) &&
    ...
    render(ctx) {
        if (this._intraGraphicAlignLines.length === 0 &&
