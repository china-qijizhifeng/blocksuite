import { assertExists, assertInstanceOf, DisposableGroup, } from '@blocksuite/global/utils';
...
import { blockElementGetter, getRootByEditorHost, isInsidePageEditor, } from '../../../_common/utils/query.js';
...
import { decodeClipboardBlobs, encodeClipboardBlobs, } from '../../clipboard/utils.js';
...
export class EdgelessClipboardController extends PageClipboard {
    ...
    constructor(host) {
        ...
        this._onCopy = (_context, surfaceSelection) => {
            const event = _context.get('clipboardState').raw;
            event.preventDefault();
            const elements = getCloneElements(this.selectionManager.selectedElements, this.surface.edgeless.service.frame);
            ...
            if (surfaceSelection[0] && surfaceSelection[0].editing) {
                ...
                if (isCanvasElementWithText(elements[0]))
                    return;
        ...
        this._onPaste = async (_context) => {
            ...
            if (surfaceSelections[0]?.editing) {
                ...
                if (isCanvasElementWithText(selectedElements[0]))
                    return;
            ...
            const point = [lastMousePos.x, lastMousePos.y];
            if (isPureFileInClipboard(data)) {
                const files = data.files;
                if (files.length === 0)
                    return;
                const imageFiles = [], attachmentFiles = [];
                [...files].forEach(file => {
            ...
            if (isUrlInClipboard(data)) {
                ...
                const { lastMousePos } = this.toolManager;
                const [x, y] = this.host.service.viewport.toModelCoord(lastMousePos.x, lastMousePos.y);
                ...
                const options = {
                    xywh: Bound.fromCenter(Vec.toVec({
                        x,
                        y,
                ...
                const id = this.host.service.addBlock(flavour, options, this.surface.model.id);
                this._rootService.telemetryService?.track('CanvasElementAdded', {
                    control: 'canvas:paste',
                    page: 'whiteboard editor',
                    module: 'toolbar',
            ...
            try {
                ...
                if (mayBeSliceDataJson === undefined)
                    return;
                const clipData = JSON.parse(mayBeSliceDataJson);
                const sliceSnapShot = clipData?.snapshot;
    ...
    _createCanvasElement(clipboardData, idMap) {
        ...
        const id = this.host.service.addElement(clipboardData.type, clipboardData);
        ...
        const element = this.host.service.getElementById(id);
        assertExists(element);
        return element;
    }
    _createCanvasElements(elements, idMap) {
    ...
    _createNoteBlocks(notes, oldToNewIdMap) {
        const { host } = this;
        const noteIds = notes.map(({ id, props, children }) => {
            delete props.index;
            assertExists(props.xywh);
            const noteId = host.service.addBlock('affine:note', props, this.doc.root.id);
            const note = host.service.getElementById(noteId);
            if (id)
                oldToNewIdMap.set(id, noteId);
            assertExists(note);
    ...
    _createEdgelessTextBlocks(edgelessTexts, oldToNewIdMap) {
        const { host } = this;
        const edgelessTextIds = edgelessTexts.map(({ id, props, children }) => {
            delete props.index;
            assertExists(props.xywh);
            const textId = host.service.addBlock('affine:edgeless-text', props, this.edgeless.surface.model.id);
    ...
    _createFrameBlocks(frames) {
        const frameIds = frames.map(({ props }) => {
            const { xywh, title, background } = props;
            const frameId = this.host.service.addBlock('affine:frame', {
                xywh,
                background,
    ...
    async _createImageBlocks(images, oldToNewIdMap) {
        const imageIds = [];
        for (const { props, id } of images) {
            const { xywh, rotate, sourceId, size, width, height, caption } = props;
            if (!(await this.host.std.collection.blobSync.get(sourceId))) {
                continue;
            }
            const imageId = this.host.service.addBlock('affine:image', {
                caption,
                sourceId,
                xywh,
                rotate,
            ...
            if (id)
                oldToNewIdMap.set(id, imageId);
            imageIds.push(imageId);
        }
        return imageIds;
    }
    async _createAttachmentBlocks(attachments) {
        const attachmentIds = [];
        for (const { props } of attachments) {
            ...
            const attachmentId = this.host.service.addBlock('affine:attachment', {
                xywh,
                rotate,
                sourceId,
                name,
    ...
    _createBookmarkBlocks(bookmarks) {
        const bookmarkIds = bookmarks.map(({ props }) => {
            const { xywh, style, url, caption, description, icon, image, title } = props;
            const bookmarkId = this.host.service.addBlock('affine:bookmark', {
                xywh,
                style,
                url,
                caption,
    ...
    _createGithubEmbedBlocks(githubEmbeds) {
        const embedGithubIds = githubEmbeds.map(({ props }) => {
            const { xywh, style, owner, repo, githubType, githubId, url, caption, image, status, statusReason, title, description, createdAt, assignees, } = props;
            const embedGithubId = this.host.service.addBlock('affine:embed-github', {
                xywh,
                style,
                owner,
                repo,
    ...
    _createYoutubeEmbedBlocks(youtubeEmbeds) {
        const embedYoutubeIds = youtubeEmbeds.map(({ props }) => {
            const { xywh, style, url, caption, videoId, image, title, description, creator, creatorUrl, creatorImage, } = props;
            const embedYoutubeId = this.host.service.addBlock('affine:embed-youtube', {
                xywh,
                style,
                url,
                caption,
    ...
    _createFigmaEmbedBlocks(figmaEmbeds) {
        const embedFigmaIds = figmaEmbeds.map(({ props }) => {
            const { xywh, style, url, caption, title, description } = props;
            const embedFigmaId = this.host.service.addBlock('affine:embed-figma', {
                xywh,
                style,
                url,
                caption,
    ...
    _createLinkedDocEmbedBlocks(linkedDocEmbeds) {
        const embedLinkedDocIds = linkedDocEmbeds.map(({ props }) => {
            const { xywh, style, caption, pageId } = props;
            return this.host.service.addBlock('affine:embed-linked-doc', {
                xywh,
                style,
                caption,
                pageId,
    ...
    _createSyncedDocEmbedBlocks(syncedDocEmbeds) {
        const embedSyncedDocIds = syncedDocEmbeds.map(({ props }) => {
            const { xywh, style, caption, scale, pageId } = props;
            return this.host.service.addBlock('affine:embed-synced-doc', {
                xywh,
                style,
                caption,
                scale,
    ...
    _createHtmlEmbedBlocks(htmlEmbeds) {
        const embedHtmlIds = htmlEmbeds.map(({ props }) => {
            const { xywh, style, caption, html, design } = props;
            const embedHtmlId = this.host.service.addBlock('affine:embed-html', {
                xywh,
                style,
                caption,
                html,
    ...
    _createLoomEmbedBlocks(loomEmbeds) {
        const embedLoomIds = loomEmbeds.map(({ props }) => {
            const { xywh, style, url, caption, videoId, image, title, description } = props;
            const embedLoomId = this.host.service.addBlock('affine:embed-loom', {
                xywh,
                style,
                url,
                caption,
    ...
    _emitSelectionChangeAfterPaste(canvasElementIds, blockIds) {
        const newSelected = [
            ...canvasElementIds,
            ...blockIds.filter(id => {
    ...
    _updatePastedElementsIndex(elements, originalIndexes) {
        function compare(a, b) {
            if (a instanceof SurfaceGroupLikeModel && a.hasDescendant(b)) {
                ...
                if (originalIndexes.get(a.id) < originalIndexes.get(b.id))
                    return -1;
    ...
    _pasteTextContentAsNote(content) {
        const edgeless = this.host;
        const { lastMousePos } = this.toolManager;
        const [x, y] = edgeless.service.viewport.toModelCoord(lastMousePos.x, lastMousePos.y);
        const noteProps = {
            xywh: new Bound(x, y, DEFAULT_NOTE_WIDTH, DEFAULT_NOTE_HEIGHT).serialize(),
        };
        const noteId = edgeless.service.addBlock('affine:note', noteProps, this.doc.root.id);
        ...
        if (typeof content === 'string') {
            splitIntoLines(content).forEach((line, idx) => {
                edgeless.service.addBlock('affine:paragraph', { text: new DocCollection.Y.Text(line) }, noteId, idx);
            });
        }
        else {
    ...
    async _pasteShapesAndBlocks(elementsRawData) {
        const { canvasElements, blockModels } = await this.createElementsFromClipboardData(elementsRawData);
        this._emitSelectionChangeAfterPaste(canvasElements.map(ele => ele.id), blockModels.map(block => block.id));
    }
    async _replaceRichTextWithSvgElement(element) {
        const richList = Array.from(element.querySelectorAll('.inline-editor'));
        await Promise.all(richList.map(rich => {
    ...
    async _edgelessToCanvas(edgeless, bound, nodes, canvasElements = [], { background, padding = IMAGE_PADDING, dpr = window.devicePixelRatio || 1, } = {}) {
        ...
        const container = rootElement.querySelector('.affine-block-children-container');
        if (!container)
            return;
        ...
        const replaceRichTextWithSvgElementFunc = this._replaceRichTextWithSvgElement.bind(this);
        ...
        const html2canvasOption = {
            ...
            onclone: async function (documentClone, element) {
                ...
                const boxShadowElements = documentClone.querySelectorAll("[style*='box-shadow']");
        ...
        const _drawTopLevelBlock = async (block, isInFrame = false) => {
            let blockElement = blockElementGetter(block, this.std.view)?.parentElement;
            const blockPortalSelector = block.flavour.replace('affine:', '.edgeless-block-portal-');
            ...
            const canvasData = await html2canvas(blockElement, html2canvasOption);
            ctx.drawImage(canvasData, blockBound.x - bound.x + padding, blockBound.y - bound.y + padding, blockBound.w, isInFrame
                ? (blockBound.w / canvasData.width) * canvasData.height
                : blockBound.h);
        };
        const nodeElements = nodes ??
            edgeless.service.pickElementsByBound(bound, 'blocks');
        for (const nodeElement of nodeElements) {
            await _drawTopLevelBlock(nodeElement);
            if (matchFlavours(nodeElement, ['affine:frame'])) {
                const blocksInsideFrame = [];
        ...
        const surfaceCanvas = edgeless.surface.renderer.getCanvasByBound(bound, canvasElements);
        ctx.drawImage(surfaceCanvas, padding, padding, bound.w, bound.h);
        return canvas;
    }
    _checkCanContinueToCanvas(host, pathName, editorMode) {
    ...
    async createElementsFromClipboardData(elementsRawData, pasteCenter) {
        const originalIndexes = new Map();
        const blockRawData = [];
        const surfaceRawData = [];
        elementsRawData.forEach(data => {
            ...
            if (blockSnapshot) {
                ...
                const serializedElement = data;
                originalIndexes.set(serializedElement.id, serializedElement.index);
                surfaceRawData.push(serializedElement);
            }
        });
        const noteSnapshots = blockRawData.filter(data => data.flavour === 'affine:note');
        const edgelessTextSnapshots = blockRawData.filter(data => data.flavour === 'affine:edgeless-text');
        const imageSnapshots = blockRawData.filter(data => data.flavour === 'affine:image');
        const frameSnapshots = blockRawData.filter(data => data.flavour === 'affine:frame');
        const attachmentSnapshots = blockRawData.filter(data => data.flavour === 'affine:attachment');
        const bookmarkSnapshots = blockRawData.filter(data => data.flavour === 'affine:bookmark');
        const embedGithubSnapshots = blockRawData.filter(data => data.flavour === 'affine:embed-github');
        const embedYoutubeSnapshots = blockRawData.filter(data => data.flavour === 'affine:embed-youtube');
        const embedFigmaSnapshots = blockRawData.filter(data => data.flavour === 'affine:embed-figma');
        const embedLinkedDocSnapshots = blockRawData.filter(data => data.flavour === 'affine:embed-linked-doc');
        const embedSyncedDocSnapshots = blockRawData.filter(data => data.flavour === 'affine:embed-synced-doc');
        const embedHtmlSnapshots = blockRawData.filter(data => data.flavour === 'affine:embed-html');
        const embedLoomSnapshots = blockRawData.filter(data => data.flavour === 'affine:embed-loom');
        ...
        const edgelessTextIds = this._createEdgelessTextBlocks(edgelessTextSnapshots, oldIdToNewIdMap);
        const imageIds = await this._createImageBlocks(imageSnapshots, oldIdToNewIdMap);
        const frameIds = this._createFrameBlocks(frameSnapshots);
        const attachmentIds = await this._createAttachmentBlocks(attachmentSnapshots);
        ...
        const embedYoutubeIds = this._createYoutubeEmbedBlocks(embedYoutubeSnapshots);
        const embedFigmaIds = this._createFigmaEmbedBlocks(embedFigmaSnapshots);
        const embedLinkedDocIds = this._createLinkedDocEmbedBlocks(embedLinkedDocSnapshots);
        const embedSyncedDocIds = this._createSyncedDocEmbedBlocks(embedSyncedDocSnapshots);
        ...
        const blockModels = [
            ...
            ...embedHtmlIds,
            ...embedLoomIds,
        ].flatMap(id => this.host.doc.getBlock(id)?.model ?? []);
        const canvasElements = this._createCanvasElements(surfaceRawData, oldIdToNewIdMap);
        ...
        canvasElements.forEach(ele => {
            const newBound = new Bound(pasteX + ele.x - oldCommonBound.x, pasteY + ele.y - oldCommonBound.y, ele.w, ele.h);
    ...
    async toCanvas(blocks, shapes, options) {
        blocks.sort(compare);
        shapes.sort(compare);
        const bounds = [];
        ...
        const canvas = await this._edgelessToCanvas(this.host, bound, blocks, shapes, options);
        return canvas;
    }
    async copyAsPng(blocks, shapes) {
        const blocksLen = blocks.length;
        const shapesLen = shapes.length;
        if (blocksLen + shapesLen === 0)
            return;
        ...
        if (window.apis?.clipboard?.copyAsImageFromString) {
            // @ts-ignore
            await window.apis.clipboard?.copyAsImageFromString(canvas.toDataURL(IMAGE_PNG));
        }
        else {
            const blob = await new Promise((resolve, reject) => canvas.toBlob(blob => (blob ? resolve(blob) : reject('Canvas can not export blob')), IMAGE_PNG));
            assertExists(blob);
            this.std.clipboard
...
function tryGetSvgFromClipboard(clipboardData) {
    const types = clipboardData.types;
