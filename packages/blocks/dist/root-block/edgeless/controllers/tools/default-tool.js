...
import { asyncFocusRichText, buildPath, } from '../../../../_common/utils/index.js';
import { handleNativeRangeAtPoint, resetNativeSelection, } from '../../../../_common/utils/index.js';
...
import { ConnectorElementModel, GroupElementModel, MindmapElementModel, ShapeElementModel, TextElementModel, } from '../../../../surface-block/element-model/index.js';
import { hideTargetConnector, moveSubtree, showMergeIndicator, } from '../../../../surface-block/element-model/utils/mindmap/utils.js';
...
export class DefaultToolController extends EdgelessToolController {
    constructor() {
        super(...arguments);
        ...
        this._updateSelectingState = () => {
            ...
            if (tools.spaceBar) {
                ...
                const [startX, startY] = service.viewport.toModelCoord(this._moveSelectionDragStartTemp[0], this._moveSelectionDragStartTemp[1]);
            ...
            const [curX, curY] = service.viewport.toModelCoord(this._dragLastPos[0], this._dragLastPos[1]);
            ...
            const set = new Set(tools.shiftKey ? [...elements, ...selection.selectedElements] : elements);
    ...
    get draggingArea() {
        if (this.dragType === DefaultModeDragType.Selecting) {
            const [startX, startY] = this._service.viewport.toViewCoord(this._dragStartModelCoord[0], this._dragStartModelCoord[1]);
            const [endX, endY] = this._service.viewport.toViewCoord(this._dragLastModelCoord[0], this._dragLastModelCoord[1]);
            return {
    ...
    _addEmptyParagraphBlock(block) {
        const blockId = this._doc.addBlock('affine:paragraph', { type: 'text' }, block.id);
        if (blockId) {
        ...
        if (this.edgelessSelectionManager.isInSelectedRect(x, y)) {
            if (this.edgelessSelectionManager.selectedElements.length === 1) {
                ...
                if (isCanvasElement(selected) &&
                    ...
                    selected.labelHitTest(this._service.viewport.toModelCoord(x, y))) {
    ...
    initializeDragState(e, dragType) {
        ...
        if (this._toBeMoved.every(ele => !(ele.group instanceof MindmapElementModel))) {
            this._alignBound = this._edgeless.service.snap.setupAlignables(this._toBeMoved);
        }
        this._clearDisposable();
        ...
        if (this.dragType === DefaultModeDragType.Selecting) {
            this._disposables.add(this._edgeless.service.viewport.viewportUpdated.on(() => {
                if (this.dragType === DefaultModeDragType.Selecting &&
                    this._dragging &&
                    !this._autoPanTimer) {
                    this._updateSelectingState();
        ...
        if (this.dragType === DefaultModeDragType.ContentMoving) {
            this._disposables.add(this._edgeless.service.viewport.viewportMoved.on(delta => {
                if (this.dragType === DefaultModeDragType.ContentMoving &&
                    this._dragging &&
                    !this._autoPanTimer) {
    ...
    _moveContent([dx, dy], alignBound, shifted, shouldClone) {
        ...
        if (this._draggingSingleMindmap) {
            const { node: currentNode, mindmap: currentMindmap, startElementBound, } = this._draggingSingleMindmap;
            const current = currentNode.element;
            const subtree = currentMindmap.getNode(current.id);
            const [x, y] = this._service.viewport.toModelCoord(this._dragLastPos[0], this._dragLastPos[1]);
            ...
            const hoveredMindmap = this._edgeless.service
                ...
                .map(el => ({
                element: el,
                node: el.group.getNode(el.id),
                mindmap: el.group,
            }))[0];
            if (hoveredMindmap) {
                const { node, element, mindmap } = hoveredMindmap;
                element.opacity = 0.8;
                const { clear, mergeInfo } = showMergeIndicator(mindmap, node, subtree, [x, y]) ?? {};
                ...
                if (!(intersects(startElementBound, bound) ||
        ...
        frame
            ? this._surface.overlays.frame.highlight(frame)
            ...
            if (!e.keys.shift &&
                ...
                this._edgeless.updateComplete
                    ...
                    if (selected.children.length === 0) {
                        ...
                        const blockElement = this._edgeless.host.view.viewFromPath('block', buildPath(selected));
                        if (blockElement) {
    ...
    onContainerDblClick(e) {
        if (this._doc.readonly) {
            ...
            if (viewport.zoom === 1) {
                // Fit to Screen
                const { centerX, centerY, zoom } = this._edgeless.service.getFitToScreenData();
    ...
    async onContainerDragStart(e) {
        ...
        elements.forEach(element => {
            ...
            else if (element.group instanceof MindmapElementModel &&
                elements.length > 1) {
    ...
    onContainerDragEnd() {
        ...
        if (this._draggingSingleMindmap) {
            if (this._hoveredMindMap && this._hoveredMindMap.mergeInfo) {
                ...
                const { node: currentNode, mindmap: currentMindmap } = this._draggingSingleMindmap;
                moveSubtree(currentMindmap, currentNode, mindmap, mergeInfo.target, mergeInfo.index, mergeInfo.layoutType);
            }
                ...
                if (subtree) {
                    MindmapElementModel.createFromTree(subtree, mindmap.style, mindmap.layoutType, this._surface.model);
                }
            }
            else {
