...
export class ShapeToolController extends EdgelessToolController {
    constructor() {
        super(...arguments);
        ...
        this._moveWithSpaceStartPos = [0, 0];
        this._moveWithSpaceShapePosTemp = null;
    ...
    _addNewShape(e, width, height) {
        ...
        const attributes = this._edgeless.service.editPropsStore.getLastProps('shape');
    ...
    _move() {
        const { _draggingArea, _moveWithSpaceStartPos, _moveWithSpaceShapePosTemp, } = this;
    ...
    onContainerDragStart(e) {
        ...
        this._draggingElement = this._service.getElementById(id);
    ...
    onContainerDragEnd() {
        ...
        if (this._draggingArea) {
            const width = Math.abs(this._draggingArea?.end.x - this._draggingArea?.start.x);
            const height = Math.abs(this._draggingArea?.end.y - this._draggingArea?.start.y);
    ...
    onPressSpaceBar(pressed) {
        ...
        if (tools.dragging) {
            if (pressed) {
                ...
                const { start: { x: startX, y: startY }, end: { x: endX, y: endY }, } = this._draggingArea;
