...
export function isSelectSingleMindMap(els) {
    return els.length === 1 && els[0].group instanceof MindmapElementModel;
}
export function isElementOutsideViewport(viewport, element, padding = [0, 0]) {
...
export function getNearestTranslation(viewport, element, padding = [0, 0]) {
    ...
    else if (elementBound.x + elementBound.w + padding[1] >
    ...
    else if (elementBound.y + elementBound.h + padding[0] >
