 ...
 */
function hasIntersection({ x: a, y: b }, { x: c, y: d }, { x: p, y: q }, { x: r, y: s }) {
...
function isInside({ x, y }, rect, buffer = 0) {
    return (x >= rect.left - buffer &&
        x <= rect.right + buffer &&
        y >= rect.top - buffer &&
        y <= rect.bottom - buffer);
}
const getNearestSide = (point, rect) => {
    const centerPoint = {
 ...
 * Licensed under MIT.
 */
export const safeTriangle = ({ zIndex = 10000, buffer = 2, idleTimeout = 40, debug = false, } = {}) => {
    ...
    return async ({ event, referenceElement, floatingElement }) => {
        ...
        if (!isLeave || event.target !== referenceElement)
            return true;
        ...
        if ((floatingSide === 'top' && mouseY >= refRect.bottom - 1) ||
        ...
        const updateSafeTriangle = (mouseX, mouseY) => {
            ...
            const areaHeight = Math.max(Math.abs(p1.y - p2.y), Math.abs(p1.y - p3.y), Math.abs(p2.y - p3.y));
            const areaWidth = Math.max(Math.abs(p1.x - p2.x), Math.abs(p1.x - p3.x), Math.abs(p2.x - p3.x));
            ...
            path.setAttributeNS(null, 'd', `M${p1.x - basePoint.x} ${p1.y - basePoint.y} ${p2.x - basePoint.x} ${p2.y - basePoint.y} ${p3.x - basePoint.x} ${p3.y - basePoint.y} z`);
        };
 ...
 * Licensed under MIT.
 */
export const safeBridge = ({ debug = false, idleTimeout = 500, } = {}) => {
    let abortController = new AbortController();
    return async ({ event, referenceElement, floatingElement }) => {
        abortController.abort();
        const newAbortController = new AbortController();
        ...
        if (!floatingElement)
            return true;
        const checkInside = (mouseX, mouseY) => {
            if (newAbortController.signal.aborted)
                return false;
            ...
            // A constant of 1 handles floating point rounding errors.
            if ((floatingSide === 'top' && mouseY > refRect.top + 1) ||
            ...
            switch (floatingSide) {
                case 'top': {
                    rectRect = new DOMRect(Math.max(rect.left, refRect.left), rect.bottom, Math.min(rect.right, refRect.right) -
                        Math.max(rect.left, refRect.left), refRect.top - rect.bottom);
                    break;
                }
                case 'bottom': {
                    rectRect = new DOMRect(Math.max(rect.left, refRect.left), refRect.bottom, Math.min(rect.right, refRect.right) -
                        Math.max(rect.left, refRect.left), rect.top - refRect.bottom);
                    break;
                }
                case 'left': {
                    rectRect = new DOMRect(rect.right, Math.max(rect.top, refRect.top), refRect.left - rect.right, Math.min(rect.bottom, refRect.bottom) -
                        Math.max(rect.top, refRect.top));
                    break;
                }
                case 'right': {
                    rectRect = new DOMRect(refRect.right, Math.max(rect.top, refRect.top), rect.left - refRect.right, Math.min(rect.bottom, refRect.bottom) -
                        Math.max(rect.top, refRect.top));
                    break;
                }
                default:
            ...
            if (inside && debug) {
                ...
                const rectDom = document.querySelector(`#${debugId}`) ??
        ...
        if (!checkInside(event.x, event.y))
            return true;
        await new Promise(res => {
            if (newAbortController.signal.aborted)
                res();
            ...
            document.addEventListener('mousemove', e => {
                clearTimeout(idleId);
                idleId = window.setTimeout(() => newAbortController.abort(), idleTimeout);
                if (!checkInside(e.clientX, e.clientY))
                    newAbortController.abort();
            }, {
                signal: newAbortController.signal,
            });
