...
export const createKeydownObserver = ({ target, inlineEditor, onUpdateQuery, onMove, onConfirm, onEsc, interceptor = (_, next) => next(), abortController, }) => {
    let query = '';
...
export const scrollbarStyle = (container) => {
    if (!container)
        throw new Error('To prevent style pollution of the whole doc, you must add a container before the scrollbar style.');
