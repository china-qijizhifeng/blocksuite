...
export function handleIndent(editorHost, model, offset = 0) {
    ...
    nextSiblings
        .filter(sibling => matchFlavours(sibling, ['affine:list']) && sibling.type === 'numbered')
        .forEach(sibling => {
    ...
    asyncSetInlineRange(editorHost, model, { index: offset, length: 0 }).catch(console.error);
}
...
function handleParagraphDeleteActions(editorHost, model) {
    ...
    else if (matchFlavours(previousSibling, ['affine:paragraph', 'affine:list'])) {
        const modelIndex = parent.children.indexOf(model);
        if ((modelIndex === -1 || modelIndex === parent.children.length - 1) &&
            parent.role === 'content')
            return false;
    ...
    if (matchFlavours(parent, ['affine:database'])) {
        ...
        return (handleParagraphOrListSibling(editorHost, model, previousSibling, parent) ||
...
function handleParagraphBlockForwardDelete(editorHost, model) {
    ...
    function handleParagraphOrList(doc, model, nextSibling, firstChild) {
        ...
        const nextBlock = getNextBlock(editorHost, model);
        if (!firstChild && !nextBlock)
            return true;
        return (handleParagraphOrListChild(doc, model, firstChild) ||
    ...
    function handleEmbedDividerCode(nextSibling, firstChild) {
        ...
        return (handleEmbedDividerCodeChild(firstChild) ||
    ...
    if (matchFlavours(parent, ['affine:database'])) {
        ...
        return (handleParagraphOrList(doc, model, nextSibling, firstChild) ||
...
function handleUnknownBlockForwardDelete(model) {
    throw new Error('Failed to handle forwarddelete! Unknown block flavours! flavour:' +
        model.flavour);
}
export function handleLineStartBackspace(editorHost, model) {
    if (handleListBlockBackspace(editorHost, model) ||
...
export function handleLineEndForwardDelete(editorHost, model) {
    if (handleCodeBlockForwardDelete(model) ||
...
//# sourceMappingURL=rich-text-operations.js.map