...
import { handleIndent, handleMultiBlockIndent, handleMultiBlockOutdent, handleRemoveAllIndent, handleRemoveAllIndentForMultiBlocks, handleUnindent, } from '../rich-text-operations.js';
import { bracketPairs } from './bracket-pairs.js';
import { hardEnter, onBackspace, onForwardDelete } from './legacy.js';
export const bindContainerHotkey = (blockElement) => {
    ...
    const _getInlineEditor = () => {
        const inlineRoot = blockElement.querySelector(`[${INLINE_ROOT_ATTR}]`);
        if (!inlineRoot) {
            throw new Error('Inline editor root not found');
        }
        return inlineRoot.inlineEditor;
    };
    const _getPrefixText = (inlineEditor) => {
    ...
    blockElement.bindHotKey({
        ...
        Enter: ctx => {
            ...
            if (!blockElement.host.contains(target))
                return;
            if (!blockElement.selected?.is('text'))
                return;
            ...
            const inlineRange = inlineEditor.getInlineRange();
            assertExists(inlineRange);
            if (!tryConvertBlock(blockElement, inlineEditor, _getPrefixText(inlineEditor), inlineRange)) {
                return true;
            }
        ...
        'Mod-Backspace': ctx => {
            if (!(blockElement.selected?.is('block') ||
                blockElement.selected?.is('text')))
                return;
            const rootElement = blockElement.closest('affine-page-root,affine-edgeless-root');
            if (!rootElement)
                return;
            {
                const [_, context] = std.command
    ...
    function tryConvertToLinkedDoc() {
        ...
        assertExists(root);
        const docBlock = blockElement.host.view.viewFromPath('block', buildPath(model.doc.root));
        assertExists(docBlock);
        const linkedDocWidgetEle = blockElement.host.view.getWidget('affine-linked-doc-widget', root.id);
