...
import { highlightCache, } from '../../code-block/utils/highlight-cache.js';
...
export class HtmlAdapter extends BaseAdapter {
    constructor() {
                ...
                `.replace(/\s\s+/g, ''),
                    ...
                    case 'affine:code': {
                        ...
                        context
                            ...
                            children: await this._deltaToHighlightHasts(text.delta, o.node.props.language),
                        }, 'children')
                            .closeNode()
                            .closeNode();
                        break;
                ...
                switch (o.node.tagName) {
                    case 'div': {
                        if (o.parent?.node.type === 'element' &&
                            ...
                            if (o.node.properties.className.includes('affine-block-children-container') ||
        ...
        this._deltaToHighlightHasts = async (deltas, rawLang) => {
            ...
            if (!rawLang ||
                ...
                rawLang === 'Text' ||
                !bundledLanguagesInfo.map(({ id }) => id).includes(rawLang)) {
                return [
                    {
                        type: 'text',
                        value: delta.insert,
            ...
            const cacheKey = `${delta.insert}-${rawLang}-light`;
            ...
            if (cache) {
                ...
                tokens = highlighter.codeToTokensBase(delta.insert, { lang }).reduce((acc, cur, index) => {
                    if (index === 0) {
                        return cur;
                    }
                    return [...acc, { content: '\n', color: 'inherit' }, ...cur];
