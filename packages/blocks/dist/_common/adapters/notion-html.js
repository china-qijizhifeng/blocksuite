...
import { hastGetElementChildren, hastGetTextChildrenOnlyAst, hastGetTextContent, hastQuerySelector, } from './hast.js';
...
export class NotionHtmlAdapter extends BaseAdapter {
    constructor() {
                ...
                switch (o.node.tagName) {
                    ...
                    case 'figure': {
                        ...
                        if (bookmark) {
                            ...
                            const bookmarkTitle = hastGetTextContent(hastQuerySelector(bookmark, '.bookmark-title'));
                            const bookmarkDescription = hastGetTextContent(hastQuerySelector(bookmark, '.bookmark-description'));
                            const bookmarkIcon = hastQuerySelector(bookmark, '.bookmark-icon');
                            const bookmarkIconURL = typeof bookmarkIcon?.properties?.src === 'string'
                    ...
                    case 'th': {
                        ...
                        const columnType = Array.isArray(columnTypeClass)
                            ...
                            : 'rich-text';
                        context.pushGlobalContextStack('hast:table:column', {
                            type: columnType,
                    ...
                    case 'tr': {
                        if (o.parent?.node.type === 'element' &&
                            ...
                            const columns = context.getGlobalContextStack('hast:table:column');
                            ...
                            hastGetElementChildren(o.node).forEach((child, index) => {
                                if (plainTable || columns[index] === undefined) {
                                    ...
                                    if (columns[index] === undefined) {
                                        ...
                                        context.pushGlobalContextStack('hast:table:children', {
                                            type: 'block',
                                            id: nanoid(),
                                            flavour: 'affine:paragraph',
                                            props: {
                                ...
                                if (hastQuerySelector(child, '.selected-value')) {
                                    ...
                                    child.type === 'element' &&
                                        child.children.forEach(span => {
                                            const filteredArray = columns[index].data.options?.filter(option => option.value === hastGetTextContent(span));
            ...
            walker.setLeave((o, context) => {
                ...
                switch (o.node.tagName) {
                    ...
                    case 'table': {
                        ...
                        const children = context.getGlobalContextStack('hast:table:children');
                        context.setGlobalContextStack('hast:table:children', []);
                        const cells = Object.create(null);
                        context
                            .getGlobalContextStack('hast:table:rows')
                            .map((row, i) => {
                            Object.keys(row).forEach(columnId => {
                                if (columns.find(column => column.id === columnId)?.type ===
                                    'select') {
                                    row[columnId].value = row[columnId].value[0];
                                }
                            });
                                ...
                                views: [
                                    {
                                        ...
                                        header: {
                                            titleColumn: columns.find(column => column.type === 'title')?.id ??
                                                '',
                                            iconColumn: 'type',
                                        },
                                    },
        ...
        this._hastToDeltaSpreaded = (ast, option = { trim: true }) => {
            ...
            switch (ast.type) {
                ...
                case 'element': {
                    switch (ast.tagName) {
                        ...
                        case 'span': {
                            return ast.children.flatMap(child => this._hastToDeltaSpreaded(child, option));
                        }
                        case 'strong': {
                            return ast.children.flatMap(child => this._hastToDeltaSpreaded(child, option).map(delta => {
        ...
        this._hastToDelta = (ast, option = { trim: true }) => {
