...
import { getElementProps, sortEdgelessElements, } from '../../root-block/edgeless/utils/clone-utils.js';
...
export function promptDocTitle(host, autofill) {
    const notification = host.std.spec.getService('affine:page').notificationService;
    if (!notification)
...
export function getNotesFromDoc(doc) {
    const notes = doc.root?.children.filter(child => matchFlavours(child, ['affine:note']) &&
...
async function renderNoteContent(card) {
    ...
    const noteChildren = notes.flatMap(note => note.children.filter(filterTextModel));
    ...
    const previewTemplate = card.host.renderSpecPortal(previewDoc, previewSpec.value);
...
function renderSurfaceRef(card) {
    ...
    const linkedDoc = card.linkedDoc;
    assertExists(linkedDoc, `Trying to load page ${card.model.pageId} in linked page block, but the page is not found.`);
    card.surfaceRefRenderer = card.surfaceRefService.getRenderer(PathFinder.id(card.path), linkedDoc);
...
async function renderPageAbstract(card) {
    ...
    const target = notes.flatMap(note => note.children.filter(child => matchFlavours(child, ['affine:image'])))[0];
    if (target) {
