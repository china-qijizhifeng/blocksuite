...
test('snapshot 1 importing', async () => {
    ...
    const snapshotFile = await fetch('https://test.affineassets.com/test-snapshot-1.zip')
        .then(res => res.blob())
        .catch(e => {
        console.error(e);
        throw e;
    });
    const [newDoc] = await transformer.importDocs(window.editor.doc.collection, snapshotFile);
    editor.doc = newDoc;
    await wait();
    const surface = newDoc.getBlockByFlavour('affine:surface')[0];
...
test('snapshot 2 importing', async () => {
    const pageService = window.editor.host.std.spec.getService('affine:page');
    const transformer = pageService.transformers.zip;
    const snapshotFile = await fetch('https://test.affineassets.com/test-snapshot-2%20(onboarding).zip')
        .then(res => res.blob())
        .catch(e => {
        console.error(e);
        throw e;
