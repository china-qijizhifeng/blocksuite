...
describe('frame', () => {
    ...
    test('frame should have title', async () => {
        const frame = service.doc.addBlock('affine:frame', {
            xywh: '[0,0,300,300]',
            title: new Text('Frame 1'),
        }, service.surface.id);
        await wait();
        const titleDom = document.querySelector(`edgeless-frame-title[data-frame-title-id="${frame}"] > .affine-frame-title`);
        ...
        const nestedFrame = service.doc.addBlock('affine:frame', {
            xywh: '[20,20,200,200]',
            title: new Text('Frame 2'),
        }, service.surface.id);
        await wait();
        const nestedTitle = document.querySelector(`edgeless-frame-title[data-frame-title-id="${nestedFrame}"] .affine-frame-title`);
        const nestedTitleRect = nestedTitle.getBoundingClientRect();
        const [nestedTitleX, nestedTitleY] = service.viewport.toModelCoord(nestedTitleRect.x, nestedTitleRect.y);
