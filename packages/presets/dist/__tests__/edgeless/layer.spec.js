...
test('change element should update layer automatically', async () => {
    ...
    service.updateElement(id, {
        index: service.layer.getReorderedIndex(service.getElementById(id), 'forward'),
    });
    expect(service.layer.layers[service.layer.layers.length - 1].type).toBe('block');
    service.updateElement(canvasElId, {
        index: service.layer.getReorderedIndex(service.getElementById(canvasElId), 'forward'),
    });
    expect(service.layer.layers[service.layer.layers.length - 1].type).toBe('canvas');
});
...
test('if the topmost layer is canvas layer, the length of canvasLayers array should equal to the counts of canvas layers', () => {
    ...
    expect(service.layer.canvasLayers.length).toBe(service.layer.layers.filter(layer => layer.type === 'canvas').length);
});
...
test('blocks should rerender when their z-index changed', async () => {
    ...
    const assertBlockElementsContent = () => {
        const blockElements = Array.from(document.querySelectorAll('.affine-edgeless-layer > edgeless-block-portal-note'));
    ...
    service.addElement('shape', {
        shapeType: 'rect',
        index: generateKeyBetween(service.getElementById(blocks[1]).index, service.getElementById(blocks[2]).index),
    });
    await wait();
    assertBlockElementsContent();
});
describe('layer reorder functionality', () => {
    ...
    test('forward', async () => {
        service.updateElement(ids[0], {
            index: service.layer.getReorderedIndex(service.getElementById(ids[0]), 'forward'),
        });
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.getElementById(ids[0])))).toBe(1);
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.getElementById(ids[1])))).toBe(0);
        await wait();
        service.updateElement(ids[1], {
            index: service.layer.getReorderedIndex(service.getElementById(ids[1]), 'forward'),
        });
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.getElementById(ids[0])))).toBe(0);
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.getElementById(ids[1])))).toBe(1);
    });
    test('front', async () => {
        service.updateElement(ids[0], {
            index: service.layer.getReorderedIndex(service.getElementById(ids[0]), 'front'),
        });
        await wait();
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.getElementById(ids[0])))).toBe(3);
        service.updateElement(ids[1], {
            index: service.layer.getReorderedIndex(service.getElementById(ids[1]), 'front'),
        });
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.getElementById(ids[1])))).toBe(3);
    });
    test('backward', async () => {
        service.updateElement(ids[3], {
            index: service.layer.getReorderedIndex(service.getElementById(ids[3]), 'backward'),
        });
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.getElementById(ids[3])))).toBe(1);
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.getElementById(ids[2])))).toBe(2);
        await wait();
        service.updateElement(ids[2], {
            index: service.layer.getReorderedIndex(service.getElementById(ids[2]), 'backward'),
        });
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.getElementById(ids[3])))).toBe(3);
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.getElementById(ids[2])))).toBe(2);
    });
    test('back', async () => {
        service.updateElement(ids[3], {
            index: service.layer.getReorderedIndex(service.getElementById(ids[3]), 'back'),
        });
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.getElementById(ids[3])))).toBe(0);
        await wait();
        service.updateElement(ids[2], {
            index: service.layer.getReorderedIndex(service.getElementById(ids[2]), 'back'),
        });
        expect(service.layer.layers.findIndex(layer => layer.set.has(service.getElementById(ids[2])))).toBe(0);
    });
});
test('indexed canvas should be inserted into edgeless portal when switch to edgeless mode', async () => {
    ...
    expect(getSurface(doc, editor).edgeless.rootElementContainer.canvasSlot.children
        .length).toBe(1);
    const indexedCanvas = getSurface(doc, editor).edgeless.rootElementContainer
        .canvasSlot.children[0];
...
test('the actual rendering order of blocks should satisfy the logic order of their indexes', async () => {
    ...
    const blocks = Array.from(edgeless.rootElementContainer.layer.querySelectorAll('[data-portal-block-id]'));
    ...
    blocks.forEach((block, index) => {
        ...
        expect(service.layer.compare(prevModel, model)).toBeLessThanOrEqual(0);
    });
});
describe('index generator', () => {
    let preinsertedShape;
    let preinsertedNote;
    beforeEach(() => {
        ...
        preinsertedShape = service.getElementById(shapeId);
        preinsertedNote = service.getElementById(noteId);
    });
