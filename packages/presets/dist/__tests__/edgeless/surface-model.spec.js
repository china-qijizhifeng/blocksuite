...
beforeEach(async () => {
    ...
    const models = doc.getBlockByFlavour('affine:surface');
    model = models[0];
    return cleanup;
});
...
describe('element model', () => {
    ...
    test('assign value to model property should update ymap directly', () => {
        ...
        const element = model.getElementById(id);
        expect(element.yMap.get('strokeColor')).toBe('--affine-palette-line-yellow');
...
describe('stash/pop', () => {
    test('stash and pop should work correctly', () => {
        ...
        const elementModel = model.getElementById(id);
...
describe('basic property', () => {
    test('empty group should have all zero xywh', () => {
        ...
        const group = model.getElementById(id);
...
//# sourceMappingURL=surface-model.spec.js.map