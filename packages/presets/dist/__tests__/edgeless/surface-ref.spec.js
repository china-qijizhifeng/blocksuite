...
describe('basic', () => {
    ...
    beforeEach(async () => {
        ...
        frame = service.addBlock('affine:frame', {
            xywh: '[0, 0, 800, 200]',
        }, service.surface.id);
        return cleanup;
    });
    test('surface-ref should be rendered in page mode', async () => {
        const surfaceRefId = doc.addBlock('affine:surface-ref', {
            reference: frame,
        }, noteA);
        editor.mode = 'page';
        await wait();
        expect(document.querySelector(`affine-surface-ref[data-block-id="${surfaceRefId}"]`)).instanceOf(Element);
    });
    test('surface-ref should be rendered as empty surface-ref-block-edgeless component page mode', async () => {
        ...
        await wait();
        const refBlock = document.querySelector(`affine-edgeless-surface-ref[data-block-id="${surfaceRefId}"]`);
    ...
    test('content in frame should be rendered in the correct order', async () => {
        ...
        editor.mode = 'page';
        await wait();
        const surfaceRef = document.querySelector(`affine-surface-ref[data-block-id="${surfaceRefId}"]`);
        const refBlocks = Array.from(surfaceRef.querySelectorAll('surface-ref-note-portal'));
        const stackingCanvas = Array.from(surfaceRef.querySelector('.stacking-canvas').children);
        expect(refBlocks.length).toBe(2);
        expect(stackingCanvas.length).toBe(1);
        expect(stackingCanvas[0].style.zIndex > refBlocks[0].style.zIndex).toBe(true);
    });
    test('content in group should be rendered in the correct order', async () => {
        ...
        expect(stackingCanvas[1].style.zIndex > refBlocks[0].style.zIndex).toBe(true);
    });
