import { BlocksUtils, Bound, getElementsBound, NoteDisplayMode, } from '@blocksuite/blocks';
import { CreateIcon, InsertBelowIcon, ReplaceIcon, } from '../../_common/icons.js';
...
const CommonActions = [
    {
        ...
        handler: async (host, content, currentTextSelection, currentBlockSelections) => {
            ...
            if (currentTextSelection) {
                ...
                if (matchFlavours(block?.model ?? null, ['affine:paragraph'])) {
                    block?.model.text?.replace(currentTextSelection.from.index, currentTextSelection.from.length, content);
                    return;
                }
            }
            await replace(host, content, data.selectedBlocks[0], data.selectedBlocks.map(block => block.model), currentTextSelection);
        },
    },
    {
        icon: InsertBelowIcon,
        title: 'Insert below',
        handler: async (host, content, currentTextSelection, currentBlockSelections, currentImageSelections) => {
            const [_, data] = host.command
                .chain()
                .getSelectedBlocks({
                currentTextSelection,
            ...
            await insertBelow(host, content, data.selectedBlocks[data.selectedBlocks?.length - 1]);
        },
    },
];
    ...
    ...CommonActions,
    {
        ...
        handler: async (host, content) => {
            ...
            if (elements.length > 0) {
                const bound = getElementsBound(elements.map(e => Bound.deserialize(e.xywh)));
...
//# sourceMappingURL=actions-handle.js.map