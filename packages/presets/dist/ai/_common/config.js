import { EDGELESS_ELEMENT_TOOLBAR_WIDGET, matchFlavours, } from '@blocksuite/blocks';
...
import { imageFilterStyles, imageProcessingTypes, textTones, translateLangs, } from '../actions/types.js';
import { getAIPanel } from '../ai-panel.js';
import { AIProvider } from '../provider.js';
import { getSelectedImagesAsBlobs, getSelectedTextContent, getSelections, } from '../utils/selection-utils.js';
import { AIDoneIcon, AIImageIcon, AIImageIconWithAnimation, AIMindMapIcon, AIPenIcon, AIPenIconWithAnimation, AIPresentationIcon, AIPresentationIconWithAnimation, AISearchIcon, AIStarIconWithAnimation, ChatWithAIIcon, ExplainIcon, ImproveWritingIcon, LanguageIcon, LongerIcon, MakeItRealIcon, MakeItRealIconWithAnimation, SelectionIcon, ShorterIcon, ToneIcon, } from './icons.js';
...
export function createImageFilterSubItem(trackerOptions) {
...
export function createImageProcessingSubItem(trackerOptions) {
...
const blockActionTrackerOptions = {
    control: 'block-action-bar',
    where: 'ai-panel',
};
const textBlockShowWhen = (chain) => {
    const [_, ctx] = chain
        .getSelectedModels({
        types: ['block', 'text'],
    })
    ...
    if (!selectedModels || selectedModels.length === 0)
        return false;
    return selectedModels.some(model => matchFlavours(model, ['affine:paragraph', 'affine:list']));
};
const codeBlockShowWhen = (chain) => {
    const [_, ctx] = chain
        .getSelectedModels({
        types: ['block', 'text'],
    })
...
const imageBlockShowWhen = (chain) => {
    const [_, ctx] = chain
        .getSelectedModels({
        types: ['block'],
    })
...
function edgelessHandler(id, generatingIcon, variants, trackerOptions) {
    return (host) => {
        if (host.doc.root?.id === undefined)
            return;
        const edgeless = host.view.getWidget(EDGELESS_ELEMENT_TOOLBAR_WIDGET, host.doc.root.id)?.edgeless;
        if (!edgeless) {
            AIProvider.slots.requestRunInEdgeless.emit({ host });
        }
        else {
            edgeless.tools.setEdgelessTool({ type: 'copilot' });
            const currentController = edgeless.tools.controllers['copilot'];
            ...
            return edgelessActionToHandler(id, generatingIcon, variants, async () => {
                ...
                const sendAttachments = selections?.selectedBlocks?.length === 1 && attachments.length > 0;
                return {
    ...
    items: [
        ...
        },
        {
            ...
            showWhen: chain => {
                ...
                if (!selectedModels || selectedModels.length === 0)
                    return false;
                return selectedModels.every(model => matchFlavours(model, ['affine:paragraph', 'affine:list']) &&
...
export function buildAIImageItemGroups() {
    return [
        {
            name: 'edit with ai',
            items: [
                {
                    ...
                    icon: ExplainIcon,
                    showWhen: () => true,
                    handler: actionToHandler('explainImage', AIStarIconWithAnimation, undefined, blockActionTrackerOptions),
                },
            ],
        },
        {
            ...
            items: [
                {
                    ...
                    icon: AIImageIcon,
                    showWhen: () => true,
                    handler: edgelessHandler('createImage', AIImageIconWithAnimation, undefined, blockActionTrackerOptions),
                },
                {
                ...
                },
                {
                    ...
                    showWhen: () => true,
                    beta: true,
                    handler: actionToHandler('generateCaption', AIStarIconWithAnimation, undefined, blockActionTrackerOptions),
                },
            ],
        },
        OthersAIGroup,
...
export function buildAICodeItemGroups() {
    return [
        {
            name: 'edit with ai',
            items: [
                {
                    ...
                    icon: ExplainIcon,
                    showWhen: () => true,
                    handler: actionToHandler('explainCode', AIStarIconWithAnimation, undefined, blockActionTrackerOptions),
                },
                {
                    name: 'Check code error',
                    icon: ExplainIcon,
                    showWhen: () => true,
                    handler: actionToHandler('checkCodeErrors', AIStarIconWithAnimation, undefined, blockActionTrackerOptions),
                },
            ],
        },
        OthersAIGroup,
