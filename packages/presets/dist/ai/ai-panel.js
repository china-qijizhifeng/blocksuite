import { AFFINE_AI_PANEL_WIDGET, AffineAIPanelWidget, Bound, ImageBlockModel, isInsideEdgelessEditor, matchFlavours, NoteDisplayMode, } from '@blocksuite/blocks';
import { assertExists } from '@blocksuite/global/utils';
import { AIPenIcon, AIStarIconWithAnimation, ChatWithAIIcon, CreateIcon, DiscardIcon, InsertBelowIcon, InsertTopIcon, ReplaceIcon, RetryIcon, } from './_common/icons.js';
...
import { copyTextAnswer, insertAbove, insertBelow, replace, } from './utils/editor-actions.js';
        ...
        handler: () => {
            ...
            doc.transact(() => {
                ...
                insertFromMarkdown(host, panel.answer, noteBlockId)
                    ...
                    if (!newNote || !matchFlavours(newNote, ['affine:note']))
                        return;
                    const newNoteBound = Bound.deserialize(newNote.xywh);
                    const bounds = [bound, newNoteBound];
                    const { zoom, centerX, centerY } = service.getFitToScreenData([20, 20, 20, 20], bounds);
