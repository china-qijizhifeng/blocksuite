    ...
    constructor(editor) {
        ...
        this._onSelectionChange = () => {
            ...
            if (!range.intersectsNode(rootElement)) {
                const isContainerSelected = range.endContainer.contains(rootElement) &&
        ...
        this._onCompositionEnd = async (event) => {
            this._isComposing = false;
            if (!this.editor.rootElement.isConnected)
                return;
            const range = this.editor.rangeService.getNativeRange();
        ...
        this._onBeforeInput = (event) => {
            ...
            if (event.inputType.startsWith('delete')) {
                if (isInEmbedGap(range.commonAncestorContainer) &&
                ...
                else if (isInEmptyLine(range.commonAncestorContainer) &&
                    tmpInlineRange.length === 0 &&
                    tmpInlineRange.index > 0) {
            ...
            transformInput(newEvent.inputType, data, ctx.attributes, newInlineRange, this.editor);
            this.editor.slots.inputting.emit();
        };
        ...
        this.mount = () => {
            ...
            this.editor.disposables.addFromEvent(eventSource, 'compositionupdate', this._onCompositionUpdate);
