...
import { assertSelection, enterInlineEditorPlayground, focusInlineRichText, getDeltaFromInlineRichText, getInlineRangeIndexRect, getInlineRichTextLine, press, setInlineRichTextRange, type, } from './utils.js';
...
test('type many times in one moment', async ({ page }) => {
    ...
    await focusInlineRichText(page);
    await page.waitForTimeout(100);
    await Promise.all('aaaaaaaaaaaaaaaaaaaa'.split('').map(s => page.keyboard.type(s)));
...
test('readonly mode', async ({ page }) => {
    ...
    await page.evaluate(() => {
        ...
        richTextA.inlineEditor.setReadonly(true);
    });
...
test('yText should not contain \r', async ({ page }) => {
    ...
    expect(message).toBe('yText must not contain "\\r" because it will break the range synchronization');
});
...
//# sourceMappingURL=inline-editor.spec.js.map