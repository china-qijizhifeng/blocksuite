...
describe('blocksuite yjs', () => {
    ...
    describe('object', () => {
        test('deep', () => {
            ...
            const { obj3 } = proxy;
            obj3.id = 'obj3';
            expect(map.get('obj3').get('id')).toBe('obj3');
            proxy.arr = [];
            expect(map.get('arr')).toBeInstanceOf(Y.Array);
            proxy.arr.push({ counter: 1 });
            expect(map.get('arr').get(0)).toBeInstanceOf(Y.Map);
            expect(map.get('arr').get(0).get('counter')).toBe(1);
        });
        test('with y text', () => {
            const ydoc = new Y.Doc();
            const map = ydoc.getMap('map');
...
//# sourceMappingURL=yjs.unit.spec.js.map