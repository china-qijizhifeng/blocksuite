...
import { NoteBlockSchema, ParagraphBlockSchema, RootBlockSchema, } from './test-schema.js';
...
function serializCollection(doc) {
    const spaces = {};
...
function waitOnce(slot) {
    return new Promise(resolve => slot.once(val => resolve(val)));
}
...
describe('basic', () => {
    it('can init collection', () => {
        ...
        const actualDoc = actual[spaceMetaId].pages[0];
...
describe('addBlock', () => {
    ...
    it('can observe slot events', async () => {
        const doc = createTestDoc();
        queueMicrotask(() => doc.addBlock('affine:page', {
            title: new doc.Text(),
        }));
    ...
    it('can set doc state', () => {
        ...
        assert.deepEqual(collection.meta.docMetas.map(({ id, title }) => ({
            id,
            title,
        })), [
            {
...
describe('deleteBlock', () => {
    it('delete children recursively by default', () => {
        ...
        const deletedModel = doc.getBlockById('1');
    ...
    it('bring children to parent', () => {
        ...
        const deletedModel = doc.getBlockById('2');
        const deletedModelParent = doc.getBlockById('1');
        doc.deleteBlock(deletedModel, {
    ...
    it('bring children to other block', () => {
        ...
        const deletedModel = doc.getBlockById('2');
        const moveToModel = doc.getBlockById('3');
        doc.deleteBlock(deletedModel, {
...
describe('getBlock', () => {
    ...
    it('can get parent', () => {
        ...
        doc.addBlock('affine:paragraph', {}, noteId);
        doc.addBlock('affine:paragraph', {}, noteId);
        const result = doc.getParent(rootModel.children[0].children[1]);
    ...
    it('can get previous sibling', () => {
        ...
        doc.addBlock('affine:paragraph', {}, noteId);
        doc.addBlock('affine:paragraph', {}, noteId);
        const result = doc.getPrev(rootModel.children[0].children[1]);
...
//# sourceMappingURL=collection.unit.spec.js.map