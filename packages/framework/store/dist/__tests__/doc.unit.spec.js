...
import { DividerBlockSchema, ListBlockSchema, NoteBlockSchema, ParagraphBlockSchema, RootBlockSchema, } from './test-schema.js';
const BlockSchemas = [
    RootBlockSchema,
...
test('trigger props updated', () => {
    ...
    const getColor = () => rootModel.yBlock.get('prop:style').get('color');
    const getItems = () => rootModel.yBlock.get('prop:items');
    ...
    expect(getItems().get(0).get('id')).toBe('1');
});
test('stash and pop', () => {
    const options = createTestOptions();
    const collection = new DocCollection(options);
...
test('selector', () => {
    ...
    const selector = block => block.flavour !== 'affine:list'
    ...
    const list1 = doc1.addBlock('affine:list', {}, note);
    ...
    const list2 = doc1.addBlock('affine:list', {}, note);
...
//# sourceMappingURL=doc.unit.spec.js.map